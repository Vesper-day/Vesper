...
The flow is entered from the product landing page's signup CTA (there is no waitlist — Vesper ships direct-to-release). It begins with a brief animated welcome sequence and proceeds through authentication (email, Google, or Apple sign-in) and archetype selection. At the archetype screen, the user chooses from six labeled tabs — Nine-to-Five Professional, Remote Worker, Student, Athlete-Focused, Founder Mode, and Mixed — with no detailed descriptions; the label is sufficient. This selection routes the user into one of two branches. Form of address, goals, and trial confirmation are deliberately deferred until after the first plan is revealed, so the user reaches a concrete plan as quickly as possible.

Users whose archetype implies an existing planning surface (generally any archetype other than one implying no prior scheduling habit) proceed through the calendar-connected branch: Google Calendar authorization, a brief universal setup step capturing wake time, bed time, and location, a single choice of one starting lifestyle module (Work and Tasks and the calendar are always on), and a short preference capture for that one module. The engine then generates a provisional first plan within approximately ninety seconds and reveals it with one prompted adjustment ("Anything I should move?"). Only after the reveal does the product collect form of address and two to three goals, then the trial confirmation — which under the card-at-signup model is also the **pricing and card-capture step** — followed by a four-to-five-screen feature tour, a dismissable "get the iOS app" overlay on web, and arrival at the day's plan. Time to first plan is under three minutes and is **unchanged by the pivot**: the card step sits after the plan reveal, not before it, so nothing is asked for money before the product has demonstrated itself. The remaining setup is progressive, and further modules are proposed one at a time over the following weeks.
...
The following documents every screen a new user encounters from first launch through the first operational day's plan. The flow is plan-first: the provisional plan is generated and revealed partway through, before form of address, goals, and the trial/card step are collected. This screen list matches the canonical fourteen-screen sequence in Layer 4.

**Direct-release pivot — what changed and what did not.** The flow's **entry** is now the product landing's signup CTA (there is no waitlist), and **Screen 12** becomes the pricing + card-capture step because the seven-day trial now requires a **card at signup** (captured, not charged; first charge $19.99 at trial end unless cancelled). The **screen order is unchanged** — the card step stays at position 12, after the first-plan reveal — and one **web-only, dismissable "get the iOS app" overlay** is added at Screen 14, on dismiss of which the user lands on the signed-in web app home (`/plan`). Nothing else in the sequence moves.
...
**Screen 12 — Trial Confirmation and Card Capture.** **Card-at-signup (supersedes the prior no-card model).** The screen states the offer plainly: one week free, then $19.99 per month, **with the card placed now and first charged at the end of the seventh day unless cancelled**, cancellable anytime. The prior headline literal — ~~"One week free. No card required."~~ — is **retired and must not ship**; its replacement is authored in build chat 035-V through the caveman/stop-slop voice gate, so no new literal is committed here. The pricing sentence "After seven days, $19.99 per month. Cancel anytime." is unchanged and still correct. A single primary button hands off to the hosted checkout — Stripe Checkout on web, StoreKit on iOS — where the card is entered; **Vesper never renders a card field of its own**. On success the flow continues to Screen 13; cancelling out of the hosted checkout returns to this screen with onboarding progress intact and the account still in trial state. Placing the card does **not** change `subscription_status`, which stays `trial` until the provider's first-charge webhook at trial end.
...
**Screen 14 — Done / Plan View (with the web-only App Store overlay).** A single screen reads "All set. Welcome to Vesper." with an "Enter" control that reveals the full day's plan. **On web only**, a dismissable "get the iOS app" overlay is presented here before the plan: it offers the App Store link in the same "iOS first. Android coming later." posture the landing states, carries a visible dismiss affordance, is dismissable by escape and backdrop, blocks nothing, and is shown once — on dismiss the user lands on the signed-in web app home (`/plan`). It replaces the retired waitlist launch-day email as the place the App Store link is delivered. Mobile skips it. Onboarding is complete. The butler line is active. The Dynamic Island Live Activity starts for the first block. The product is operational.
...
**Trial onset.** The trial begins the moment the user completes the trial-confirmation step in onboarding. The `subscription_status` field is set to `trial`. **A payment method IS collected at that step** (superseding "No payment method is collected") — captured via Stripe Checkout on web or StoreKit on iOS and **not charged**. The trial window is seven days from that timestamp. The full feature set is accessible without restriction throughout the trial.
...
**Day-five optional payment-method capture — RETIRED by card-at-signup.** *(Kept for lineage.)* The prior model offered, on trial day five, an opt-in prompt to save a payment method for a frictionless conversion the next day. The card is now placed at signup, so there is nothing to capture on day five and the prompt is not built.

**Day-six one-tap conversion prompt — RETIRED by card-at-signup.** *(Kept for lineage.)* Every user now has a method on file from signup, so the day-six touch is the plain informational trial reminder: it states that the trial ends tomorrow and that the card on file is charged $19.99 at trial end unless the user ends the trial. No discount, no countdown, no pre-authorization prompt. Users who take no action proceed to the end-date prompt the following day.
...
**Trial conversion — Continue.** The user taps Continue. **Under card-at-signup the method is already on file, so nothing is collected here** — no payment sheet, no re-entry of details; Continue confirms and the existing method is charged $19.99. On successful payment, `subscription_status` transitions to `active` and the first billing cycle begins. The butler-voice confirmation is "Welcome aboard."

**Trial conversion — End.** The user taps End. `subscription_status` transitions to `read_only`. No payment is taken and the card on file is never billed. The read-only state and subsequent archive proceed identically to the post-cancellation path described below.
...
Every new account begins in a seven-day free trial. **Payment information IS collected at signup** (superseding "No payment information is collected at signup"): at the trial-confirmation step the card is captured through the hosted checkout and **not charged**, and the first charge of $19.99 falls at the end of the seventh day unless the user cancels first. The trial starts at that step and runs seven days. The trial provides complete access to every V1 feature without restriction. Every module is available to activate, Google Calendar sync is available, and the only usage cap is a soft limit of two plan generations per local day to protect margin on non-converting trial users. There is no reduced-functionality trial tier; the trial is the full product.

The trial period is tracked in the user's subscription status as "trial" and its expiration is driven by a trial expiration timestamp set at account creation. Three reminder touchpoints fire during the final days of the trial. At two days before expiration, the plan includes a butler-voice note: "Two days left in your trial. Anything you'd like to ask before then?" At one day before expiration, a second note appears: "Your trial ends tomorrow. Shall I keep things running?" Both reminders are in-plan and in-app only; no push notification is sent for trial reminders. *(The day-six one-tap Apple Pay / Stripe Link conversion prompt is **retired by card-at-signup** — the card is already on file, so there is nothing to capture. The day-six touch is the plain trial reminder, which states that the card on file is charged $19.99 at trial end unless the user ends the trial.)* On the expiration date itself, the user's first plan view of the day presents a clean full-screen prompt. The screen displays "Your week is up." and two buttons: Continue and End. Pressing Continue confirms the charge against the card already on file — no payment sheet and no re-entry of details. Pressing End transitions the account to read-only state immediately and the card is never billed.
...
**Under card-at-signup, the hosted payment surfaces are met at signup, not here.** iOS users complete the StoreKit 2 flow at the onboarding trial-confirmation step, which handles the subscription entirely within Apple's infrastructure using the payment method on the user's Apple ID; web users complete a Stripe-hosted Checkout at the same step. Pressing Continue at trial end therefore confirms against the method already on file rather than opening a payment sheet, and the user returns to their plan within a single interaction session.
...
The production domain is registered at approximately Project Week 22 (start of Phase 5 internal alpha). Before that point, the application runs against localhost during development and Vercel preview URLs (*.vercel.app) for any external surface. The product landing page locked in Layer 4's Marketing Visual Language section is built during Phase 4 and deployed to a preview URL until the domain is registered, then re-deployed to the production domain at Week 22. **Direct-release pivot:** the landing is the launched product's front page and its CTA routes into real signup, so the Week 22 domain go-live is the point at which the page starts accepting real signups — there is no waitlist and no eight-week pre-launch holding period.
