...
Eight features ship at V1. Each is described below at the level of precision needed to implement it.
...
At onboarding, the engine seeds the user's base profile from one of six structure archetypes: Nine-to-Five Professional, Remote Worker, Student, Athlete-Focused, Founder Mode, and Mixed. The archetype provides the starting template shape — a skeleton of when certain block types tend to appear relative to each other — which the engine then personalizes over the first one to two weeks as real behavioral data accumulates. The user does not see the archetype label after onboarding; it functions as a silent initialization parameter.
...
The seven modules are independently toggleable subsystems that collectively cover the domains of daily life the product schedules. All seven are enabled by default at onboarding. A user who toggles a module off removes it entirely from the mobile interface; re-enabling requires navigating to settings. On the web interface, disabled modules remain visible at lower prominence to give users who monitor multiple domains a broader view. Each module is described in full below.

**Work and Tasks.** The work module is the task management subsystem. Users create tasks with a title, an estimated duration, an optional deadline, and a priority. The engine places task chunks into focus blocks throughout the available day, continuously reshuffling as the calendar changes. At V1, task placement is single-day focused: the engine plans today and previews tomorrow but does not perform cross-week rebalancing. Cross-week rebalancing arrives at V2. The work module is on by default and is the module with the highest expected usage rate across all user archetypes.

**Fitness.** The fitness module schedules workout blocks from a curated template library of approximately 150 workouts. Each template is tagged by goal (strength, cardio, fat loss, maintenance, mobility), equipment required (gym, home, bodyweight, dumbbells, full barbell rack), time available (15, 30, 45, or 60 minutes), and difficulty level. The engine selects the appropriate template each morning based on the user's stated goal, the energy slider value, estimated recovery state from the prior day's workout, and the equipment available at the user's registered location. Users can swap a suggested workout with a single command ("give me something shorter" or "I want a home workout today") and the engine presents the next-best match without requiring the user to browse the library. The fitness module is on by default.

**Nutrition.** The nutrition module schedules meal blocks from a recipe template library of approximately 300 recipes. Recipes are tagged by dietary restrictions (vegetarian, vegan, gluten-free, dairy-free, and similar), cuisine style, prep time, and macronutrient profile. The engine selects daily meals based on dietary restrictions captured at onboarding, the user's stated cooking time tolerance, recorded food dislikes, and macronutrient targets if set. Grocery list generation is automatic: the week's meal plan converts to an itemized shopping list, which routes to the user's nearest appropriately stocked grocery store via the local intelligence layer. Hydration tracking lives inside the nutrition module as a sub-feature; it does not have its own module toggle. The nutrition module is on by default.

**Sleep.** The sleep module manages the full bedtime arc. It schedules a wind-down routine block in the evening (configurable; the default is 30 minutes before the user's stated bedtime), surfaces a bedtime reminder when the wind-down period begins, and provides the integrated morning alarm. The alarm is designed for one-tap dismissal without requiring the phone to be unlocked; it presents two edge-to-edge buttons labeled SNOOZE and STOP. After the alarm is dismissed, the energy check-in slider appears as the next screen — not before, so that the alarm dismissal is never gated. The product enforces quiet hours derived from the user's stated sleep target (bedtime to wake time), during which no notifications fire. The sleep module is on by default.

**Errands and Home.** The errands module manages to-dos, recurring chores, and one-off errands. Recurring chores (trash day, laundry, plant watering) are entered once and appear automatically on the appropriate day each week. One-off errands (pharmacy, dry cleaning, returns) are added on demand. When multiple errands share a geographic proximity, the local intelligence layer groups them into a batched sequence and presents the optimal routing order rather than a flat list. The module also manages non-grocery shopping reminders as reminder-based nudges, not as a checkout surface. The errands module is on by default.

**Medication.** The medication module tracks pill timing, dose schedules, supplement scheduling, and refill reminders. The user manually enters each medication with a name, dose, time of day, and frequency at module setup. This module is the only module for which push notifications are permitted beyond payment failure alerts, because the health stakes of a missed medication dose justify the intrusion. Medication data is never shared with any third party, never reported to health insurance, and not synced to health platforms at any planned version. The medication module is on by default.
...
The local intelligence layer is the cross-cutting capability that surfaces location-aware suggestions throughout the user's day. It is not a standalone screen; it manifests as contextual enrichment across every module where geography is relevant.

At V1, the layer provides four types of location-aware output. First, grocery routing: the week's meal plan converts to a shopping list that routes to the user's nearest grocery store with adequate stock, with optional Instacart or UberEats delivery integration for users who prefer delivery. Second, restaurant and cafe suggestions: the Yelp Fusion API provides restaurant recommendations for meals the user is not cooking at home, and a separate cafe surface filters by wifi availability and quiet rating for focus blocks the user wants to take away from their home or office. Third, errand batching: geographically clustered errands are grouped and presented in optimal sequence so the user can complete multiple stops in a single trip. Fourth, traffic-aware buffers: the engine automatically inserts a calculated transit buffer between any two location-bound events based on real-time routing data, so a 10 AM meeting at a different location never collides with a 9 AM block that ends at home.

Cost containment governs how the local intelligence layer runs. Places data is aggressively cached — the user's nearest five grocery stores, three gyms, five cafes, and three pharmacies are computed at onboarding and refreshed weekly rather than per-query. Objective queries such as distance calculations use rule-based math rather than AI inference. AI inference is reserved for subjective queries such as "best vibe match for tonight's dinner." Free-tier API usage (Yelp Fusion's 5,000-call-per-day free tier, Google Places monthly free credits, OpenStreetMap and Nominatim as fallback geocoding) covers V1 user volume without meaningful cost. Mapbox routing is used over Google Maps routing for materially lower per-call pricing at equivalent quality.

### Pillar 4: Calendar Dual Surface
...
The flow begins with a brief animated welcome sequence and proceeds through authentication (email, Google, or Apple sign-in), a form-of-address selection (Sir, Madam, or no honorific), and archetype selection. At the archetype screen, the user chooses from six labeled tabs — Nine-to-Five Professional, Remote Worker, Student, Athlete-Focused, Founder Mode, and Mixed — with no detailed descriptions; the label is sufficient. This selection routes the user into one of two branches.

Users whose archetype implies an existing planning surface (generally any archetype other than one implying no prior scheduling habit) proceed through the calendar-connected branch: Google Calendar authorization, location capture, sleep target entry, two to three primary goal fields, module enable selection with all seven modules on by default, brief per-module preference capture, trial confirmation, first plan generation, a four-to-five-screen feature tour, and arrival at the day's plan. Total time for this branch is five to eight minutes.

Users who do not have an existing planning surface proceed through the no-existing-plan branch, which adds a built-in calendar walkthrough between archetype selection and location capture. The walkthrough is three screens explaining how to enter fixed weekly events — work hours, classes, recurring appointments — into the product's native calendar. All subsequent steps match the calendar-connected branch. Total time for this branch is ten to fifteen minutes.
...
The following documents every screen a new user encounters from first launch through the first operational day's plan.
...
**Screen 2 — Authentication.** Three vertically stacked options: Continue with Google, Continue with Apple, Continue with email. A small note below the options reads "We never sell your data." The user selects one option and completes the standard authentication flow for that provider. On completion, the account is created with `subscription_status` set to `trial` and `trial_ends_at` set to the current timestamp plus fourteen days.

**Screen 3 — Form of Address.** Headline: "How should I address you?" Three stacked options: Sir, Madam, No honorific. The user's selection is stored and applied to every butler-voice line throughout the product that contains an honorific placeholder. A footnote reads "You can change this anytime in settings."

**Screen 4 — Archetype Selection.** Headline: "What does your day usually look like?" Six tab-style labels on a single screen without descriptive text: Nine-to-Five Professional, Remote Worker, Student, Athlete-Focused, Founder Mode, Mixed. The user's selection determines the onboarding branch (calendar-connected or no-existing-plan) and initializes the plan engine's base profile template.

**Screen 5A — Google Calendar Connect (calendar-connected branch).** Headline: "Let's start with your calendar." A primary button reads "Connect Google Calendar." A less-prominent "Skip for now" link appears below. Tapping the primary button initiates the standard Google OAuth flow. On successful authorization, the application reads the user's upcoming events and stores them as fixed blocks. Users who skip proceed to the built-in calendar, which they can populate later.

**Screen 5B — Built-in Calendar Walkthrough (no-existing-plan branch).** A three-screen tutorial sequence with the headline "Let's set up your schedule." The three screens guide the user through entering fixed weekly events — work hours, classes, recurring appointments — into the product's native calendar interface. Each screen focuses on one entry type with an inline example. This walkthrough replaces Screen 5A for users in the no-existing-plan branch.

**Screen 6 — Location Capture.** Headline: "Where are you based?" A Mapbox map view with a location pin centered on the device's current location (if permission is granted) or on a default city center (if not yet granted). A "Use current location" button appears at the top; manual address entry is available below. The permission prompt copy reads: "Vesper uses your location to find nearby groceries, gyms, and cafes." On confirmation, the application computes and caches the user's nearest grocery stores, gyms, cafes, and pharmacies.

**Screen 7 — Sleep Target.** Headline: "When does your day begin and end?" Two time pickers: Wake time and Bed time. A note below reads "These set your quiet hours." The values initialize the quiet-hours window and drive the sleep module's bedtime reminder logic.

**Screen 8 — Goals.** Headline: "What are your two or three priorities right now?" Three open text fields. A "Skip" link is available. Goal entries seed the plan engine's priority context and appear as suggested starting points during the weekly planning flow.

**Screen 9 — Module Selection.** Headline: "Which of these matters to you?" Seven toggle rows, one per module, each with a one-line description. All seven are on by default. The user can toggle any module off here; modules can also be toggled later in settings.

**Screen 10 — Module Preferences.** A brief per-module preference capture for each module that is enabled. This screen adapts based on which modules are on. For the fitness module: goal type (strength, cardio, fat loss, maintenance), equipment available (gym, home, bodyweight), days per week. For the nutrition module: dietary restrictions, cooking time tolerance, notable dislikes. For the sleep module: target sleep hours (pre-filled from the wake and bed times set in Screen 7). For the errands module: any recurring chores to pre-populate. For the medication module: medication entry flow (name, dose, time, frequency) if enabled. Finance and work modules have no preference capture at this stage; they are configured through normal use.

**Screen 11 — Trial Confirmation.** Headline: "Two weeks free. No card required." Body: "After fourteen days, $19.99 per month. Cancel anytime." A single primary button reads "Start my free trial." No secondary option appears; users who do not want to continue simply exit the application (their account persists in trial state and can be resumed).

**Screen 12 — First Plan Generation.** A full-screen loading state using a butler-appropriate animation (candle flame or ember pulse rather than a spinner) while the plan engine generates the user's first day. The butler line reads "Your first day is ready." when generation completes. If generation fails, the fallback line and graceful degradation sequence described in Pillar 1 apply.

**Screens 13–16 — Feature Tour.** A four-screen tour covering the four core interactions a new user must understand: how to mark a block complete (tap the block, then the completion control), how to use the natural-language input surface (tap the italic butler line at the bottom of the plan view), where the Dynamic Island integration lives (shown via a device illustration), and where to find the weekly planning view (the calendar icon in the navigation). Each screen has a "Next" control and a "Skip tour" link. Skipping advances to Screen 17 immediately.

**Screen 17 — Plan View.** The user arrives on the day's plan. Onboarding is complete. The butler line is active. The Dynamic Island Live Activity starts for the first block. The product is operational.
...
**Morning alarm.** At the user's configured wake time, the alarm fires. The alarm screen presents two edge-to-edge buttons: SNOOZE and STOP. Tapping either dismisses the alarm without requiring the phone to be unlocked. SNOOZE delays the alarm by a configurable interval (default ten minutes). STOP dismisses and advances to the energy check-in.
...
**Morning brief.** The day's plan displays with the next three blocks visible above the fold, today's weather summary, and the commute time to the first location-bound event. The user can scroll to see the full day. The butler line renders a morning greeting line from the contextual pool. The Dynamic Island activates for the current or upcoming first block.
...
**Evening summary.** At or shortly after the day's last scheduled block, the product surfaces an end-of-day summary: blocks completed, blocks missed or skipped, the day's energy reading, and a preview of tomorrow's plan. The sleep module, if enabled, surfaces the wind-down reminder block at the configured offset before bedtime.
...
**Step 1 — Prior Week Review.** A summary of last week's completion rate, presented as a percentage of blocks completed. If the reason for skipped or missed blocks is determinable from available data (a rescheduled block that was never completed, for instance), brief annotations appear alongside the blocks in question. On the Standard tier at V1, this review is completion-focused only; mood and energy trend analysis arrives with the paid Optimizer tier at V1.5.
...
**Trial onset.** The trial begins the moment the user completes onboarding and taps "Start my free trial." The `subscription_status` field is set to `trial`. No payment method is collected. The trial window is fourteen days from that timestamp. The full feature set is accessible without restriction throughout the trial.

**Trial reminders.** A scheduled job checks each morning for users whose trial ends in three days, one day, or zero days. At each checkpoint, a transactional email is sent in butler voice, and an in-app ambient butler line cue and a soft plan-view banner appear. The three-day-out in-app copy: "Three days left in your trial. Anything you'd like to ask before then?" The one-day-out copy: "Your trial ends tomorrow. Shall I keep things running?" The end-date copy: "Your two weeks are up." with two unstyled buttons — Continue and End. No countdown timer, no persuasion copy, no discount offer appears at any trial reminder touchpoint.
...
Plan generation happens on a schedule and in response to triggers. The morning plan is the primary generation event. A Cloudflare Worker pre-warms the AI context cache for each active user at 5:30 AM in the user's local timezone — thirty minutes before the most common 6:00 AM alarm window — so that when the user submits the energy slider after waking, the plan appears quickly rather than waiting for a full cold inference call. The energy slider submission is the final trigger that unlocks the morning plan: the engine receives the energy value, applies its intensity scaling to the pre-warmed context, and completes the final plan assembly.
...
Vesper's default posture is aggressive restraint. The product does not treat push notifications as a general-purpose communication channel. Two categories of push notifications are permitted at V1 and no others.

The first permitted category is medication reminders. Because the health consequences of a missed dose can be meaningful, push notification is warranted despite the general anti-interruption posture. Medication push notifications fire at user-specified times. If the specified time falls within the user's quiet hours (derived from their stated sleep schedule), the notification shifts to the next non-quiet window, and the user is warned of this behavior at medication module setup.
...
| Trial: 3 days remaining | "Three days left in your trial. Anything you'd like to ask before then?" |
...
The principles applied in practice are: tactical empathy (acknowledging the user's situation before any request or prompt), calibrated questions (using "how" and "what" questions rather than yes/no questions where the user has genuine agency), labeling (using "It seems..." or "It sounds..." sparingly to acknowledge emotional or practical state), loss framing reserved for genuine decision moments (trial end, cancellation, account deletion — not sprinkled throughout routine copy), and commitment and consistency acknowledgment (surfacing what the user has already built or configured rather than asking for new commitment from a standing start).
...
The module schedules three meal blocks by default: breakfast, lunch, and dinner. Each block shows the recipe name and estimated preparation time. Expanding the block reveals the full recipe: ingredient list with quantities, step-by-step preparation instructions, and a macronutrient summary. The user can swap any meal with a command in the butler input surface; the engine substitutes the next best match from the filtered candidate set. Grocery list generation is automatic: the engine aggregates the ingredient lists from the current week's scheduled meal plan and produces an itemized shopping list. That list routes to the user's nearest appropriately stocked grocery store via the local intelligence layer, surfacing the store name and address in the plan. The grocery list is a static generated document at V1; it does not integrate with any grocery delivery service natively, though the local intelligence layer can surface a link to the store's Instacart page where available.
...
The medication module tracks pill timing, dose schedules, supplement timing, and refill reminders. Unlike all other modules, setup requires manual data entry: each medication is entered with a name, a dose amount and unit, a time or times of day, and a frequency (daily, twice daily, every other day, weekly, and similar). The module stores this information in the user's encrypted profile. Medication data is never shared with third parties, never reported to health insurance systems, and not synchronized to Apple Health or any other health platform at V1 or in any planned version. The medication module is on by default at onboarding but its medication list is empty until the user populates it; an empty list produces no plan blocks and no reminders.

For each configured medication, the module schedules a reminder block in the daily plan at the specified time and fires a push notification at that time. Medication reminders are one of only two categories for which push notifications are permitted at V1 — the other being payment failure alerts — because the health stakes of a missed dose justify the intrusion into the product's otherwise notification-restrained posture. The push notification copy is minimal: "Time for your [medication name]." The user marks the reminder complete by tapping the notification or the plan block.
...
### 6.6 Errands and Local Intelligence

The errands module manages the household and personal logistics layer: recurring chores, one-off errands, and geographic task batching. Recurring chores are entered once with a name, a frequency (daily, weekly, every two weeks, monthly), and a day-of-week anchor where applicable. Once entered, recurring chores appear automatically in the plan on their scheduled day without requiring further user action. One-off errands are added on demand through the butler input surface or the errands module directly, with an optional due date and an optional location tag.

The local intelligence layer activates when multiple pending errands share geographic proximity. The layer identifies errand clusters — groups of stops within a reasonably walkable or drivable radius of each other — and presents the cluster to the user as a single batched errand block with stops listed in optimal routing order rather than as individual unconnected items. The routing order is computed using Mapbox distance and traffic data, with OpenStreetMap and Nominatim as fallback sources. A traffic buffer is automatically added to the errand block's estimated duration when real-time or historical traffic data indicates congestion on the route. The butler line that surfaces the batch suggestion reads: "Three stops, best done in this order."

Location data for errand batching is derived from two sources: the user's manually tagged errand locations and the cached places database populated at onboarding. At onboarding completion, the local intelligence layer pre-fetches and caches the user's nearest grocery stores, gyms, cafes, and pharmacies so that subsequent errand clustering does not require a live API call per use. Cached places refresh weekly in the background. Users who move or add a new registered location trigger a manual cache refresh. The initial pre-fetch runs silently at onboarding completion and resolves within seconds on a standard mobile connection.

The errands module also manages non-grocery shopping reminders. These surface as reminder blocks in the plan on the user-specified day with the item name and any attached notes. The product does not integrate with any retailer or e-commerce platform; shopping reminders are not clickable purchase links and do not connect to Amazon, Target, or similar services. The module surfaces the reminder at the right time and no further.
...
Every new account begins in a fourteen-day free trial. No payment information is collected at signup; the trial starts immediately upon account creation at the conclusion of the onboarding flow without any payment commitment from the user. The trial provides complete access to every V1 feature without restriction — all seven modules are enabled by default, the local intelligence layer is active, Google Calendar sync is available, and no usage caps or feature gates apply during the trial period. There is no reduced-functionality trial tier; the trial is the full product.

The trial period is tracked in the user's subscription status as "trial" and its expiration is driven by a trial expiration timestamp set at account creation. Three reminder touchpoints fire during the final days of the trial. At three days before expiration, the plan includes a butler-voice note: "Three days left in your trial. Anything you'd like to ask before then?" At one day before expiration, a second note appears: "Your trial ends tomorrow. Shall I keep things running?" Both reminders are in-plan and in-app only; no push notification is sent for trial reminders. On the expiration date itself, the user's first plan view of the day presents a clean full-screen prompt. The screen displays "Your two weeks are up." and two buttons: Continue and End. Pressing Continue advances to the payment flow. Pressing End transitions the account to read-only state immediately.
...
The three-month success criteria established in the product's foundational planning are: five thousand total signups and one hundred paying subscribers at ninety days post-public launch. These targets are the diagnostic threshold for evaluating product-market fit at the first major checkpoint. They are directional targets, not contractual commitments; the diagnostic table below specifies the analytical response to outcomes above and below the threshold. All diagnostic actions at every checkpoint are founder-discretion responses. No automated interventions, pricing changes, or experiments are triggered by any outcome.

### North-Star Metric

Day-30 retention rate on trial cohorts is the north-star metric. It is defined as: of users who began their free trial in week W, the percentage who are still active — either as paying subscribers or in the trial-overlap edge case — on day thirty post-signup. This metric is the earliest reliable signal of product-market fit because it captures whether users find sufficient value to remain through the trial-to-paid transition, which is the central behavioral test of the product.

Day-30 retention is preferred over signup volume, monthly recurring revenue, or activation rate as the north star because it is the metric most resistant to top-of-funnel manipulation. An influx of low-quality signups inflates volume without affecting D30 retention. A premium price improves MRR per converter but may erode D30 retention through pricing pressure. The north star keeps the product accountable to genuine value delivery rather than acquisition mechanics.

### Activation and Retention Definitions

The following definitions are the precise specifications for analytics funnel construction and have no ambiguity about which event combinations define each metric.

**Activation.** A user is activated when they complete the onboarding flow, generate their first daily plan, and mark at least one block on that plan complete, all within twenty-four hours of their initial signup. In event terms, activation is the funnel: signed_up → onboarding_completed → first_plan_generated → block_completed, with the signed_up and block_completed event timestamps separated by no more than eighty-six thousand four hundred seconds.

**Day-1 retention.** A user is retained at day one when they log into the application within twenty-four hours of their initial signup and view a generated plan at that login. In event terms, D1 retention is satisfied when a plan_viewed event fires within twenty-four hours of the signed_up event.

**Day-7 retention.** A user is retained at day seven when they log into the application on day six or day seven post-signup, have generated at least one plan in the prior seven days, and have completed at least one block in the prior seven days. In event terms, D7 retention is satisfied when a plan_viewed event fires on day six or seven, a plan_generated event has fired in the prior seven days, and a block_completed event has fired in the prior seven days.

**Day-30 retention.** A user is retained at day thirty when either they have an active subscription status on day thirty — meaning they converted past the fourteen-day trial and are currently paying — or they are still within the fourteen-day trial window on day thirty in the trial-overlap edge case and have logged in within the last three days.

### Three-Month Diagnostic Checkpoint Table

The following table is the founder-facing diagnostic framework for evaluating progress against the three-month success criteria. It contains diagnostic guidance, not automated alerts or code triggers.

| Checkpoint | Observed Outcome | Diagnostic Action |
|---|---|---|
| Month 1 (Project Week 34) | 1,000 or more signups, 15 or more paying | On track. Continue organic strategy. No paid spend. |
| Month 1 | 500 to 999 signups, 5 to 14 paying | Slight underperformance. Audit channel performance: which channels drove the launch-week spike and which have died. Continue with working channels. |
| Month 1 | Under 500 signups | Significant top-of-funnel issue. Diagnose whether the Product Hunt launch underperformed, whether Hacker News picked up, and whether Reddit posts are ranking. Iterate on channel mix or run a second launch wave with refined messaging. |
| Month 1 | Over 2,000 signups | Overperformance. Hold paid spend. Monitor activation and conversion to confirm quality matches volume. |
| Month 3 (Project Week 42) | 5,000 or more signups, 100 or more paying | Success criteria met. Begin V1.5 build. Consider whether to trigger a paid acquisition test. |
| Month 3 | 5,000 or more signups, under 50 paying | Funnel break between signup and paid conversion. Activation rate or trial-to-paid conversion is the issue, not top-of-funnel. Audit onboarding drop-off in PostHog. Audit the value delivered during trial. Consider whether perceived value at $19.99 is misaligned. |
| Month 3 | Under 2,000 signups, under 50 paying | Product-market fit signal absent. Pause feature work. Run ten to twenty user interviews with signed-up-but-not-converted users and never-signed-up waitlist holdouts. Consider pivot, repositioning, or re-launch. |
| Month 3 | 25,000 or more signups, 500 or more paying | Breakout. Re-evaluate the indie versus startup-track decision. Consider whether to take outside funding or accelerate hiring. |
