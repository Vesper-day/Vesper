...
### MODULES-TAB NAV REFACTOR + METHOD-B SCAFFOLD (addendum chats added; not yet run)
...
**Status:** **ADD-A + ADD-B landed.** ADD-A (nav refactor) + ADD-B (nutrition method-B scaffold: `food_log_entries` migration 25, web routes + web/mobile page, AI recipe-modify reuse via `@vesper/ai` `modifyRecipe` / prompt `v1-20260824`, nutrition card in the catalog; offline gate + DB smoke green — see the Last-updated summary at top). ADD-C (fitness method-B scaffold) remains **specified but not yet run**. The master reordered-sequence table is **untouched** (no EO added). ADD-A is now the **live SOURCE OF TRUTH for module surface locations** — `docs/MODULE_MOUNT_CONTRACT.md`; 013's tab bar and the 060/061 original mount paths are **superseded** by the new Modules-tab tree (below).
...
- **Scaffold tables (thin, deep columns deferred):** `food_log_entries` **AUTHORED by ADD-B as migration 25** (live allocation from `MIGRATION_NUMBER_ALLOCATION.md` Block 4-7 range — next free integer was 25, NOT the TECHNICAL_SPEC's tentative #26). `lift_log_entries` (ADD-C, tentative #27) still unbuilt. Food-search / AI recipe-modify / workout-schedule / tailored-generation need **no new table** (confirmed — ADD-B's food-search reads `recipe_templates`, recipe-modify is stateless AI). ADD-C will author its migration + `.down.sql` when it runs.
