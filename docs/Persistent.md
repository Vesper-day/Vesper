...
### DIRECT-RELEASE PIVOT — WAITLIST RETIRED, CARD-AT-SIGNUP TRIAL
**Owner:** Docs-only pivot pass (this one) for the recorded intent; **the surfaces themselves are owned by their existing chats** — 093-V (landing + CTA), 032-V/032-W (the auth entry the CTA lands on), 035-V/035-W (pricing + card-capture step), 036-V/036-W (App Store overlay + `/plan` terminal). **OPEN FOLLOW-UP — a future addendum chat owns converting the shipped checkout to card-at-signup:** the landed Stripe Checkout (083 lineage) is configured `subscription_data: { trial_period_days: 0 }` with no card-at-signup path [code: `apps/web/app/api/v1/subscription/operations.ts`], i.e. it charges immediately; that chat converts it — and the StoreKit path (085/086) — to **card captured at signup, not charged, first charge $19.99 after the 7-day trial**. That chat is **not authored this pass**: no `PHASE_4_BUILD_PLAN.md` entry, no EO, named here only.
**Relevant-to:** **093-V**, **093-W** (retired), **031** (retired waitlist half), **092** (retired waitlist emails); the onboarding chats **032-V / 032-W / 035-V / 035-W / 036-V / 036-W**; the subscription / checkout chats **083** (lineage, shipped), **084** (Stripe webhooks), **085 / 086 / 086a / 087 / 088** (StoreKit + Apple notifications); **039 / 040** (the `/plan` flow terminal — recorded only, their scope is unchanged); **072** (trial-reminder worker) and **089-V / 089-W** (the day-six prompt, now retired as a conversion device); **094** (landing SEO/OG); the future card-at-signup rework addendum chat; any chat reading `LAYER_5_BUSINESS_MONETIZATION.md`, `LAYER_6_LAUNCH_GROWTH.md`, `DESIGN_STRATEGY.md`, `PRD.md` §3.1/§3.4/§7, or `LAYER_4_EXPERIENCE_IDENTITY.md` Screen 12.
**Status:** **Open — docs recorded, no code written.** This pass was **DOCS-ONLY**: no code, no migration, no component, no schema. The master reordered-sequence table and every EO are **UNTOUCHED**. Every affected chat entry was rewritten **in place** (no addendum sections, no ADD-style banners) because none of the `-V` halves has run. **No new chat entry was created** — every surface in the new flow found an existing owner.
**Detail:**

- **The pivot.** Vesper ships **direct-to-release. There is NO waitlist.** The landing at `apps/web/app/(marketing)/page.tsx` is the **launched product's front page**. Its primary CTA is a "start / join" action routing to **real signup** — not email capture, not `POST /waitlist`.
- **The flow.** landing CTA → sign up → pricing + card-capture checkout → a dismissable "get the iOS app" overlay → on dismiss, the signed-in web app home (`/plan`).
- **Surface → owning chat** (all **rewritten-existing**; none new):
  | Surface | Owner | Kind |
  |---|---|---|
  | Landing + section-5 signup CTA | **093-V** | rewritten-existing |
  | CTA → auth entry (sign-up) | **032-V** (dest. `(onboarding)/sign-in`); built surface is chat **010** `(auth)/sign-in` [code] | rewritten-existing |
  | Pricing + card-capture checkout (PRD Screen 12) | **035-V** (presentation) / **035-W** (wiring) | rewritten-existing |
  | Dismissable App Store overlay (PRD Screen 14, web only) | **036-V** (presentation) / **036-W** (dismiss → `/plan`) | rewritten-existing |
  | Signed-in web app home = flow terminal | **039** / **040** (`/plan` [code]) — recorded only, scope unchanged | rewritten-existing |
- **The trial model — CARD AT SIGNUP (operator decision; OVERRIDES the prior "no card required" wording).** The 7-day free trial **requires a card at signup**: captured during signup/checkout, **NOT billed until the trial ends**; first charge **$19.99 at trial end unless cancelled**. Pricing literal on the landing is **unchanged**: "One week free. After that, $19.99 per month." $19.99/month, no permanent free tier. Applied consistently across `LAYER_5`, `LAYER_6`, `PRD` and `LAYER_4` Screen 12. Consequences recorded in the docs: the **day-five payment-capture prompt** and the **day-six one-tap conversion prompt** are **retired** (nothing left to capture); the **trial-end screen collects nothing** — the card is already on file, so Continue confirms and charges rather than opening a payment sheet, and End never bills; `subscription_status` stays `trial` until the provider's first-charge webhook.
- **Disclosure obligation (not optional).** Because inaction now results in a charge, the card must be disclosed in the same plain register and visual weight as the price — on the landing **and** again at the capture step — and cancelling before trial end stays one obvious step. A card taken quietly fails Layer 4's 24-hour-satisfaction bar; disclosed plainly it clears it. Final CTA and disclosure copy is authored in **093-V / 035-V** through the caveman/stop-slop voice gate; **no new copy literal was committed by this docs pass**.
- **031 is RETIRED, not orphaned-pending.** No keep-inert-vs-cleanup weighing: the waitlist is removed and `POST /api/v1/waitlist` + the `waitlist` table are **dead surface with no consumer**. 031 is **landed** (EO 9), so its entry is marked retired rather than rewritten as unbuilt scope; the shipped files stay in the tree, unrouted-to. **Read the scope precisely:** only the **waitlist** deliverable retires — 031's **referral** deliverables (`POST /referral/track`, `GET /referral/code`) are **LIVE**, still consumed by 095-V/095-W, and its CD-flag **F3** determination is referral-side and stays consumed.
- **093-W is RETIRED** (not repurposed) — the CTA is a plain route link, so there is nothing to wire. It never had a master-table row, so no EO is removed.
- **092's waitlist emails are RETIRED** — the nurture ("Building.") and both launch-day variants are not authored. The App Store link they would have carried is delivered in-product by the 036 overlay; the Android honesty note moves to the landing's "iOS launching shortly" line.
- **CHECKOUT REWORK — the one real build consequence.** The shipped Stripe checkout charges immediately (`trial_period_days: 0`, no card-at-signup path) and therefore **cannot serve the new Screen-12 step as it stands**. 035-W **consumes** the reworked checkout; it does **not** rework it. See the Owner line: a future addendum chat owns the conversion of both the Stripe path and the StoreKit 085/086 path. Nothing was rewritten in 083's entry this pass.
- **Survives unchanged (do not "fix"):** the five locked landing sections in order (1 Hero interactive ~60s sample-day demo, canned, no account; 2 What Vesper does; 3 Modules — seven; 4 How it works; 5 Pricing and signup); the RICH immersive Three.js hero; reduced-motion + no-WebGL fallback; 107/107a tokens + primitives; the mobile perf budget; the launch-day channel mix (Product Hunt / HN / IH / DN / Reddit / directories / newsletter pitches / AMA); the content strategy; the referral program; the activation and retention definitions; the Android communication posture; the plan-first fourteen-screen onboarding **order** (the card step stays at Screen 12, after the plan reveal).
- **Known unreconciled references (flagged, NOT edited this pass — out of the stated doc scope):** `docs/TECHNICAL_SPEC.md` still documents the `waitlist` table (§16), the `20260601000009_waitlist_and_referrals.sql` migration, the waitlist RLS/index notes, and the `subscription_data: { trial_period_days: 0 }` checkout config; `PHASE_4_BUILD_PLAN.md` chat **009** still configures a Cloudflare-edge rate limit on `/api/v1/waitlist` (now protecting a dead endpoint). These are factually true of the shipped code and are left alone; the checkout-config line is the rework chat's to change.
...
### DESIGN-TRACK OVERHAUL — CLEAN POSTURE + FABLE MODEL (SUPERSEDED by the RICH re-overhaul — see the flag directly below)
...
**Status:** **SUPERSEDED — do not follow this flag.** The clean-posture + Fable-model decision was **reversed** by the Design-Track Re-Overhaul (ADD-D landed — the flag immediately below). Kept here only for lineage: the posture is now RICH/immersive on the single build model, the Fable carve-out is retired, `cinematic` is KEPT (its removal cancelled), and the tooling + shipped-surface polish shipped. The bullets below are historical.
...
### DESIGN-TRACK RE-OVERHAUL — RICH POSTURE + SINGLE BUILD MODEL (ADD-D LANDED; tooling + shipped-surface polish shipped)
**Owner:** The design-track `-V` chats going forward (now authored **RICH / animated / immersive on the SINGLE build model** — Fable carve-out RETIRED — per the priority order); the operator (install the user-only skills). No build-track follow-up owed by ADD-D itself.
**Relevant-to:** every not-yet-run `-V` visual half (032-V, 033-V, 034-V, 035-V, 036-V, 041-V, 044-V, 046-V, 089-V, 093-V, 095-V); any chat consuming `DESIGN_STRATEGY.md`, `DESIGN_SYSTEM.md`, `VISUAL_DIRECTION_BANK_v2.md`, or `CSP_NOTES.md`; any chat touching `packages/ui/src/tokens.ts`/`tailwind.ts`/`DesignTokens.swift`, the `apps/*/components/ui` primitives, the plan/day surfaces, or the web CSP; any chat wiring lenis/gsap/vanta+three/react-bits/Playwright
**Status:** **LANDED (docs + tooling + polish; NO migration/DB/schema).** Reverses the clean-posture + Fable flag above. Offline gate green (web test 48 files/6 DB-skip, mobile 34/202, type-check web+mobile+ui, lint web+mobile, full turbo build 6/6). The 11 `-V` halves are **re-scoped in docs but not yet run** — they now target the rich posture on the single build model.
**Detail:** Authoritative text: `PHASE_4_BUILD_PLAN.md` → Addendum → "Design-Track Re-Overhaul" + chat ADD-D; the rich-posture banner + §4.1 hero in `DESIGN_STRATEGY.md`; the CLAUDE.md standing note; `DESIGN_SYSTEM.md` §3b; `CSP_NOTES.md`.
- **Posture.** RICH / animated / immersive UI built from **code libraries** (restrained-clean no longer the ceiling), on the **SINGLE build model** (Fable / ~$100 carve-out RETIRED). AI-image/video GENERATION pipeline STAYS retired — richness comes from libraries, not generated stills. Reduced-motion → instant MANDATORY on every motion surface. Persuasion ethics (§2) + scorekeeping-as-absence (§3) UNCHANGED.
- **Tooling.** `@vesper/web` deps: `lenis@^1.3.26`, `gsap@^3.15.0`, `vanta@^0.5.24`, `three@^0.185.1` (+ dev `@types/three`). **react-bits = copy-in via jsrepo CLI, NOT installed** (the npm `react-bits` is an unrelated lib). Playwright already dev dep + config; browsers are user-run `npx playwright install`; no new spec (no served surface). **CSP: NO change** — audited three/vanta/gsap/lenis = zero `eval`/`new Function`, WebGL shader compile is GPU-side; Decision 13's `'unsafe-eval'` exclusion intact, `next.config.ts` untouched. If three worker-loaders (Draco/KTX2) are ever adopted → minimal `worker-src blob:`.
- **Loosened token-freeze (KEPT disciplined).** NEW motion/immersive tokens allowed as NEW NAMES only: `boxShadow` raised/floating/press/glow, easing `emphasized`/`overshoot`, `backdropBlur.veil` (in tokens.ts + tailwind.ts + DesignTokens.swift `VesperElevation`/curves). `cinematic` band + easing **KEPT** (removal cancelled). Existing names/values unchanged → shipped consumers (BlockCard, BlockTimeline, subscription, calendar) unaffected; never inline a hex/px/ms.
- **NEW primitives.** Web `Reveal` (motion.tsx — cinematic-band emphasized entrance, reduced-motion → instant) + `SmoothScroll` (Lenis provider, reduced-motion → native scroll). Both exported from `apps/web/components/ui/index.ts`.
- **Shipped surfaces lifted to the rich bar.** Web core (Card/Button/BlockRow/ButlerLine), web part-2 (TextField/Select/Toggle/SegmentedControl focus-glow), plan surfaces web+mobile (BlockCard/BlockTimeline/PlanSkeleton/PlanEmpty), mobile Card (`shadow-raised`), settings/integrations page. Colocated primitive tests kept green (additive token-name classes, no inline literals). `/week` + `/settings` index pages are still route **stubs** — nothing shipped there to polish yet.
- **Doc deprecation split.** `VISUAL_DIRECTION_BANK_v2.md` RE-ACTIVATED for **MOTION/immersive-staging ONLY** (its AI-generated-imagery parts stay retired). `ASSET_MANIFEST_KICKOFF.md` + `docs/REEL_REFERENCE_SHEETS/` **STAY deprecated** (AI-image GENERATION not resurrected).
- **USER prereqs.** 3 Claude Code skills — taste, web-design-guidelines, image-to-code — + `npx playwright install`. `awesome-design` was dropped by the operator (all repo mentions removed); do not re-add.
- **Priority order (single build model).** ADD-D shipped design system + plan/day → onboarding (032-V/035-V/036-V) → plan/day detail (041-V/046-V) → Modules tab + top module pages → landing/referral (093-V/095-V) → subscription/rest (089-V/033-V/034-V/044-V).
