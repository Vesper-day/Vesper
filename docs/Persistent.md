...
**Status:** Open — the addendum chats are **specified but not yet run** (docs-only pass). The master reordered-sequence table is **untouched** (no EO added). Original chats 060/061 carry one-line supersede pointers atop their sections; 013's tab bar and the 060/061 mount paths are now **partially stale**.
