...
### APPLE ASSN V2 WORKER — PART 1 landed (087); part 2 (088) + Apple cutover OPEN
**Owner:** Chat 088 (part-2 types: DID_FAIL_TO_RENEW → past_due, + any remaining §8 Apple types); the Cutover operator (wrangler deploy + App Store Connect webhook URL + `SUPABASE_DB_URL`/`SENTRY_DSN` secrets); Chat 074 (delayed_jobs reconcile consumer)
**Relevant-to:** 088; any chat touching subscription state, the subscriptions row, the Apple verify pipeline, or the reconcile-job queue; the Cutover chat
**Status:** Open — part-1 worker shipped + green (full offline gate + live-DB smoke 5/5). Inert until Cutover deploy. Part-2 types NOT handled here.
...
- **DID_FAIL_TO_RENEW is a PART-2 (088) type — audit-only here** (recorded, NO transition, 200). Any unknown notificationType is likewise audit-only.
