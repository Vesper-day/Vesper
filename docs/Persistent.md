...
- **Scaffold tables (thin, deep columns deferred):** `food_log_entries` (#26) + `lift_log_entries` (#27) in TECHNICAL_SPEC. Food-search / AI recipe-modify / workout-schedule / tailored-generation need **no new table** (stated). Each addendum chat that runs will author the migration + `.down.sql` at that time (none authored this pass).
