...
> **⚠ DESIGN-TRACK RE-OVERHAUL — RICH / ANIMATED / IMMERSIVE POSTURE (supersedes the earlier clean-posture banner and any clean-posture language below).** The design track **returns to the rich, animated, immersive vision** — built from **code libraries** (Lenis smooth-scroll, GSAP, Vanta + Three.js WebGL, react-bits) that plug into the build model, **not** from an AI-image/video generation pipeline. The target is a **rich, animated, immersive modern-app experience**: expressive motion, immersive staging, layered depth — restrained-clean is no longer the ceiling. **Reduced-motion → instant fallback is preserved on every motion surface.** The **AI-generated-imagery / video GENERATION pipeline stays retired** (no Google Flow, no manor stills, no reels) — the richness comes from code libraries, not generated art. Where any earlier clean-posture wording appears below (e.g. a "clean standard-layout hero"), **the rich posture governs.** The persuasion-with-integrity theses (§2), the 24-hour ethical bar, and the scorekeeping-as-absence discipline (§3) are **unchanged.** Design execution **returns to the single build model** — the Fable / ~$100-budget design-track carve-out is **retired**; the `-V` visual halves are authored **rich on the single build model** when they run. Authority: `docs/PHASE_4_BUILD_PLAN.md` → Addendum → "Design-Track Re-Overhaul" (the KEEP/DEPRECATE split, the tooling list, the loosened token-freeze, and the priority order). The committed 107/107a token **values are unchanged** by this overhaul.
