...
> **CARD-AT-SIGNUP TRIAL — OVERRIDES THE "NO CARD REQUIRED" MODEL THROUGHOUT THIS DOCUMENT.** Operator decision, taken with the direct-release pivot (`docs/Persistent.md` → "DIRECT-RELEASE PIVOT"). The seven-day free trial now **requires a card at signup**: the card is captured during the signup / onboarding checkout step and is **not billed until the trial ends**; the **first charge is $19.99 at the end of the seventh day unless the user cancels first**. Nothing else about the trial changes — seven days, full feature access, the two-plan-generations-per-day trial cap, the reminder cadence, and the two-button trial-end screen all stand. Every "no card required" / "no payment method is collected" statement below is **superseded**; passages are kept and marked rather than deleted, so the reasoning that produced the earlier position stays legible. The **web conversion entry** is now the product landing's signup CTA (there is no waitlist — see `LAYER_6_LAUNCH_GROWTH.md`), and the shipped Stripe Checkout (`subscription_data: { trial_period_days: 0 }`) needs rework to a card-at-signup-with-trial configuration; that rework is owned by a **future addendum chat** named in the Persistent flag.

## Working Direction From Prior Layers

The product is a Life OS for young professionals, launching in the United States only at V1, with a butler-tone voice and warm-dark aesthetic. The business model is a one-week free trial converting to paid, with no permanent free tier. **The trial requires a card at signup** (superseding the earlier no-card-upfront position); the card is captured but not charged, the trial includes every V1 feature, and the **first charge of $19.99 falls at the end of the seventh day unless the user cancels first**. The trial-end flow surfaces butler-voice reminders at two days before, one day before, and on the end date, with two clean options (continue or end) on the end-date screen. The post-trial and post-cancellation states transition through a seven-day read-only continuation mode, then a thirty-day archive, then hard delete. Cancellation is deliberately frictionless, with no retention modals, no last-minute discount offers, and no adversarial confirmation prompts. A win-back survey arrives forty-eight hours after cancellation via email, framed as a no-oriented question that respects the user's exit decision. The realistic AI cost is approximately $1.00 to $1.50 per active paying user per month, with $1.20 as the planning midpoint. This is derived from Sonnet 4.6 ($3/$15 per million input/output tokens) and Haiku 4.5 ($1/$5) against a blended cold/warm cache pattern at 15 to 20 active days per month. Earlier $0.30 to $0.50 figures used an understated output-token count and an engagement assumption (5 to 7 days per month) consistent with a churning user rather than an engaged one. Stripe Checkout, Customer Portal, and Pricing Tables are the hosted payment infrastructure choices locked in Layer 3. The founder operates anonymously, with anonymity preserved through the build phase and into public launch.
...
**Card at signup.** *(This paragraph previously read "No card required upfront" and is **superseded** by the operator's card-at-signup decision; the original reasoning is preserved below because the tradeoff it names is real and now runs the other way.)* The card is placed during signup and first charged at trial end unless cancelled. **The superseded reasoning:** "The friction reduction at signup outweighs the conversion penalty at trial end. Requiring a card upfront converts more trial users to paid because the default action becomes 'do nothing and get charged,' but the signup conversion rate drops materially. For an indie launch optimizing for top-of-funnel signups and word-of-mouth, no-card-required is correct." **What the reversal accepts:** a materially lower signup rate at the top of the funnel, in exchange for a materially higher trial-to-paid rate — and it accepts an obligation, not just a tradeoff. Because inaction now results in a charge, the card must be disclosed in the same plain register and the same visual weight as the price, on the landing and again at the capture step, and cancellation before trial end must stay one obvious step. A card taken quietly would fail Layer 4's 24-hour-satisfaction bar; disclosed plainly, it clears it.

**End-of-trial soft prompt with one-tap pay — SUPERSEDED by card-at-signup.** *(Kept for lineage.)* The prior model surfaced a day-six soft prompt offering one-tap conversion via Apple Pay on iOS or Stripe Link on web, existing purely to recover the conversion that a no-card trial gives away. With the card already on file from signup that prompt has nothing left to do: there is no method to capture and no checkout to shortcut. **What replaces it is a reminder, not a conversion device** — the day-two, day-one and day-zero trial reminders (below) stand unchanged, and the honest addition the pivot requires is that the reminders state which way inaction falls (the card on file is charged at trial end unless the user ends the trial). No countdown, no discount, no urgency chrome; the two-button trial-end screen is unchanged.
...
**Trial onset.** The user arrives from the product landing's signup CTA and creates an account via email magic link, Google OAuth, or Apple Sign In. The `trial_started_at` field on the user row is set to the current timestamp, and `trial_ends_at` is set to seven days forward. The `subscription_status` field is set to `trial`. **A payment method IS collected during onboarding** (superseding "No payment method is collected during onboarding"): at the trial-confirmation step the user is handed to Stripe Checkout on web or StoreKit on iOS, the card is captured and **not charged**, and `subscription_status` stays `trial` — the transition to `active` still happens on the provider's first-charge webhook at trial end.
...
**Day-five optional payment capture — RETIRED by card-at-signup.** *(Kept for lineage.)* The prior model offered, on trial day five, an opt-in prompt to save a payment method for a frictionless conversion the next day (Apple Pay sheet on iOS, Stripe Setup Intent on web). The card is now placed at signup, so there is nothing to capture on day five and the prompt is not built.

**Day-six one-tap conversion prompt — RETIRED by card-at-signup.** *(Kept for lineage.)* The prior model showed a one-tap conversion prompt on day six to users who had saved a method on day five. Every user now has a method on file from signup, so the day-six touch is the plain informational trial reminder already specified above — it states that the trial ends tomorrow and that the card on file is charged $19.99 at trial end unless the user ends the trial, and it offers no discount and no countdown. Users who take no action proceed to the end-date screen the following day.

**Trial-to-paid conversion.** On the trial end date, the user is presented with two options: continue or end. **Under card-at-signup, continue collects nothing** — the method is already on file, so continue confirms and the existing method is charged $19.99 rather than routing to a payment sheet; end transitions the account to the seven-day read-only continuation state and the card on file is never billed. The choice is presented in butler voice without persuasion friction: "Your week is up." with two unstyled buttons. No retention copy, no discount offer, no countdown timer.
...
**Referral referee incentive.** Users who arrive via a referral link receive the standard seven-day trial (no trial-length exception) plus a fifty percent discount on their first paid month, mirroring the referrer's credit. This keeps the trial uniformly seven days while making referral two-sided. (See Layer 6 Referral Program.)
...
**State 1: trial.** Active during the seven-day trial window. User has full access. A payment method is **on file from signup** but **no charges are made** in this state. `trial_ends_at` drives transitions out of this state.
...
**Blended customer acquisition cost on a six percent trial-to-paid conversion rate.** *(Derivation retained as the conservative planning case. **Card-at-signup should move this number up and the non-converter count down** — a card on file converts by default rather than by decision — but no revised figure is stated here, because none can be derived from a source this document holds; the six percent case is kept precisely so the economics are not re-planned on an unsourced estimate.)* At six percent conversion (the planning midpoint for an indie B2C consumer subscription app with an opt-in trial; five percent baseline plus an estimated one to two percentage points from the now-retired day-six soft prompt), every paying customer is paired with approximately 15.7 non-converting trials. Sunk trial AI cost across those non-converters at approximately $0.45 each is approximately $7.07 in trial AI spend per paying customer acquired through organic top-of-funnel. This is in addition to whatever paid acquisition cost is incurred for the funnel itself; at V1 paid acquisition is held in reserve and not actively spent. The implied CAC purely from trial overhead is approximately $7 per paying customer. The fifteen to twenty-five percent conversion rates appearing in earlier drafts were sourced from B2B SaaS benchmarks (First Page Sage, OpenView) that do not apply to consumer indie products; the applicable benchmark is ChartMogul 2026's 8.9 percent average for opt-in trials across 200 SaaS products, with the indie B2C subset below that average.
...
**Experiment 3: Card-at-signup versus no-card-required.** *(Direction reversed — card-at-signup is now the V1 baseline and no-card is the challenger.)* Test the shipped card-at-signup flow against a no-card-required flow. Hypothesis: card-at-signup loses on signup conversion and wins on trial-to-paid conversion; net new paying users is approximately equivalent, which is exactly why this is the experiment that decides it rather than an argument. Measure signup conversion, trial-to-paid conversion, and total paying users per traffic dollar.
...
**Card-required-upfront trial.** ~~Rejected per Layer 4.~~ **REVERSED — this is now the V1 model** (operator decision, direct-release pivot). The original objection stands as a constraint on *how* it ships, not on *whether*: "Auto-charge is a common pattern that converts well but reads as dishonest given the marketing language ('free trial' while requiring payment information upfront)." The honesty problem it names is answered by disclosure, not by omission — the landing and the capture step both state plainly that a card is placed now and first charged $19.99 at trial end unless cancelled, in the same visual weight as the price, and cancelling before trial end stays one obvious step. A card taken quietly would fail Layer 4's 24-hour-satisfaction bar; a card taken in the open clears it.
