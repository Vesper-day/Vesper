...
**Load at session start:** LAYER_4_EXPERIENCE_IDENTITY.md (full file, with attention to the Persuasion Principles Quietly Applied section and the scorekeeping discipline it establishes); LAYER_5_BUSINESS_MONETIZATION.md and LAYER_6_LAUNCH_GROWTH.md (the conversion and retention surfaces — trial, referral, product landing, cancellation); PRD.md (product contract); the built application shells in `apps/web/app/` and `apps/mobile/app/` so the strategy is grounded in the surfaces that already exist.
...
### Chat 031 — Waitlist and Referral APIs — WAITLIST SURFACE **RETIRED** (landed; referral surface live)

*Block 4 · EO 9 · 🔵 · Skills: — · ⇄ 030 · Window N · CD-flags: F3 C*

> **RETIRED — waitlist half. Direct-release pivot.** Vesper ships direct-to-release; **there is no waitlist**. `POST /api/v1/waitlist` and the `waitlist` table are **dead surface with no consumer**: the landing's section-5 form that was to call it is replaced by a plain signup CTA routing to the auth entry (093-V), and 093-W — the chat that would have bound the form to this endpoint — is retired. The route, its schemas, its `addToWaitlist` operation and its integration test all **shipped** (this chat is landed, EO 9) and are **left in the tree, unrouted-to and unreferenced by any surface**. This entry is **not** rewritten as unbuilt scope — it is a landed chat whose waitlist deliverable the pivot retired.
>
> **Scope of the retirement — read precisely.** Only the **waitlist** deliverable retires. The **referral** deliverables of this same chat are **LIVE and consumed**: `POST /api/v1/referral/track` and `GET /api/v1/referral/code` shipped [code] and remain the referral surface that **095-V** (the `/r/[code]` referee landing) and **095-W** (referral attribution + mint-on-active, EO 87) build on, with the Referral Program in `LAYER_6_LAUNCH_GROWTH.md` unchanged by this pivot. Do not read "031 retired" as retiring referrals.
>
> **F3 stays consumed.** The CD-flag F3 determination this chat recorded is a **referral-side** determination (`referral_credits.beneficiary_role` count-scoping, see `docs/CHAT_031_RESOLUTION_RECORD.md`), so it survives the waitlist retirement intact and 095-W still inherits it resolved.
...
**Goal (as built, EO 9):** Implement POST /waitlist (public, rate-limited at the edge) — **now retired, see the banner above**; POST /referral/track (public, sets attribution cookie and redirects) — **live**; and GET /referral/code (authenticated) — **live**. The waitlist insert returns 409 if the email exists. The referral track endpoint validates the code against users.referral_code (the column is added in chat 095 if not already present in the schema audit from chat 006).

**Output:**
- ~~`apps/web/app/api/v1/waitlist/route.ts` — POST handler that validates email format, checks for duplicates, inserts; 409 on duplicate~~ — **RETIRED: shipped, now dead surface with no consumer** (the file and its colocated `schemas.ts` / `addToWaitlist.ts` / integration test remain in the tree)
...
> **DESIGN-TRACK RE-OVERHAUL RE-SCOPE — see Addendum → "Design-Track Re-Overhaul."** This not-yet-run `-V` half runs on the **single build model** (the Fable carve-out is retired) and targets the **rich / animated / immersive** posture built from code libraries (Lenis, GSAP, Vanta + Three.js, react-bits). The Screen-1 "four-second sequential reveal" is **restored as a rich animated entrance** (GSAP / react-bits timeline; Considered/cinematic-band tokens; **reduced-motion → instant** mandatory); the immersive/cinematic wording in the Goal/Output/Impl-notes below is active direction again. The AI-generated-imagery / video GENERATION pipeline stays retired (richness from code libraries, not generated art). The build model authors the styled component code directly (Option 1). 106/107/107a token + primitive values are unchanged.

**Load at session start:** PRD §3.1 (Onboarding Flow, Screens 1–3); LAYER_4_EXPERIENCE_IDENTITY.md (Onboarding Flow section, Screens 1, 2, 3); the design system tokens and primitives from chats 107 and 107a; chats 010, 011 (existing auth surfaces being repositioned, as visual reference only); chat 017 (voice gate) for the welcome/honorific copy; **chat 093-V (the landing whose primary CTA lands on this flow — direct-release pivot).**
...
**Direct-release pivot — this flow is the landing's CTA destination.** Vesper ships direct-to-release with no waitlist, so the 093-V landing's primary signup CTA routes **here**, into real signup. This half therefore owns the **web entry seam**: the repositioned auth surface must be reachable as a first-visit destination from a cold marketing click (no prior session, no onboarding state), and its onboarding-context layout must read as "start Vesper," not as "return to Vesper." The route this half establishes for the repositioned sign-in surface (`apps/web/app/(onboarding)/sign-in/page.tsx`) is the **contract 093-V's CTA links to** — if it moves, 093-V's `href` moves with it. No new chat owns the CTA→auth hop; it is one `href` on 093-V's side and this destination on ours.
...
> **DESIGN-TRACK RE-OVERHAUL RE-SCOPE — see Addendum → "Design-Track Re-Overhaul."** This not-yet-run `-V` half runs on the **single build model** (the Fable carve-out is retired) and targets the **rich / animated / immersive** posture built from code libraries (Lenis, GSAP, Vanta + Three.js, react-bits): expressive motion, immersive staging, layered depth — with **reduced-motion → instant** mandatory. The AI-generated-imagery / video GENERATION pipeline stays retired (richness from code libraries, not generated art). The build model authors the styled component code directly (Option 1). 106/107/107a token + primitive values are unchanged.
...
**Goal:** Build the static, mocked form surfaces for screens 7 (sleep target with two time pickers), 8 (goals with three open text fields), 9 (module toggles with all seven modules, six default ON and finance default OFF), 10 (per-module quick preferences for the enabled modules), and 11 (trial confirmation, **now the pricing + card-capture step**). All forms are visually complete and mocked — persistence, the push-permission prompt timing, the card capture, and the trial-state writes are deferred to 035-W. The module-preference forms reflect the 111-corrected notification-preferences shape so the wiring half persists without field drift.

**Direct-release pivot — the trial-confirmation screen becomes the pricing + card-capture step.** The trial model is now **card-at-signup**: the card is captured during signup and **not billed until the trial ends**; the first charge is $19.99 at trial end unless cancelled. The prior "One week free. No card required." framing is **superseded** and must not be presented. This half owns the **presentation** of that step — the pricing recap ("One week free. After that, $19.99 per month.", the pricing literal, unchanged), the plain statement that a card is placed now and first charged at trial end, the cancel-anytime line, and the primary action that hands off to checkout — all authored through the caveman/stop-slop gate. **Author no card fields:** payment details are collected by the hosted provider surface (Stripe Checkout on web, StoreKit on iOS), never by a Vesper-authored form, so this half builds the framing and the hand-off affordance plus the returning/mocked success and cancelled-checkout states — not a card input. **Placement:** this step stays at its committed position in the PRD §3.1 fourteen-screen sequence (Screen 12, after the first-plan reveal, form of address, and goals). The pivot changes what the screen says and does, not the plan-first screen order.
...
- `apps/web/app/(onboarding)/trial/page.tsx` and `apps/mobile/app/(onboarding)/trial.tsx` — the **pricing + card-capture step** presentation: the pricing literal, the card-at-signup disclosure (card placed now, first charge $19.99 at trial end unless cancelled), the cancel-anytime line, and the primary hand-off action; plus the mocked post-checkout **success** and **cancelled/abandoned-checkout** return states. No card fields are authored here — payment detail entry belongs to Stripe Checkout (web) / StoreKit (iOS)
...
**End-of-session checks:** Screens 7–11 render on both web and mobile for every archetype, composed from 107/107a primitives, with all copy cleared through the voice gate. The module toggles present six default-ON and finance default-OFF. The per-module preference forms reflect the 111-corrected notification-preferences shape. **The trial screen presents the card-at-signup model — pricing literal, card-placed-now / first-charge-at-trial-end, cancel anytime — with no "no card required" wording anywhere on it and no card input field; its mocked success and cancelled-checkout return states both render.** No persistence, trial-state write, checkout call, or push-prompt firing is present (deferred to 035-W).
...
**Direct-release pivot — this half also wires the card capture.** Under the card-at-signup model the trial screen hands off to the real checkout: on web to Stripe Checkout via the landed `POST /api/v1/subscription/checkout` (chat 083), on iOS to StoreKit (chats 085/086). On return, the success path proceeds into the rest of onboarding and the cancelled/abandoned path returns the user to the 035-V cancelled state without losing any onboarding progress. **Hard prerequisite — not owned here:** the landed web checkout is configured `subscription_data: { trial_period_days: 0 }` with no card-at-signup path [code: `apps/web/app/api/v1/subscription/operations.ts`], i.e. it charges immediately and therefore **cannot serve this step as it stands**. Converting it (and the StoreKit path) to card-at-signup-with-trial is owned by a **future addendum chat**, recorded as a named open follow-up in `Persistent.md` → "DIRECT-RELEASE PIVOT." 035-W consumes that converted checkout; it does not rework it.
...
- **Card-capture hand-off wiring:** the primary action calls the (reworked) checkout entry point — Stripe Checkout on web via chat 083's route, StoreKit on iOS via chats 085/086 — and handles both returns: success advances onboarding, cancel/abandon returns to the 035-V cancelled state with onboarding progress intact. The card is captured, **not charged**; the first $19.99 charge falls at trial end unless the user cancels first

**Implementation notes:** The trial confirmation does NOT change subscription_status (the handle_new_user trigger already set it to `trial` at sign-up per Tech Spec); the confirmation step only sets the trial_started_at timestamp and records acknowledgment. **Card-at-signup does not change that either** — capturing a card leaves the user in `trial`; the transition to `active` still happens on the provider's first-charge webhook at trial end (chat 084 on web, 086 on iOS), so no new state is introduced by the pivot. The trial_ends_at field is stored at the same time so downstream queries (chat 072 trial-reminder worker, chat 089 day-6 prompt) read a stable end-of-trial timestamp. The push-permission prompt timing is the architectural decision encoded here: it fires after module preferences are captured but before plan generation, so the user has agency over the prompt timing. This wiring half adds no new copy (the degraded-state banner copy came through the gate in 035-V), so it carries no 🎩 flag.
...
**End-of-session checks:** A new user can complete all five screens for each archetype with live persistence. The `user_profiles.base_profile` JSONB and `modules_enabled` JSONB are populated correctly and without field drift from the 111-corrected shape. The trial confirmation sets `trial_started_at` and stores `trial_ends_at`. **A card placed at this step is captured and NOT charged; `subscription_status` stays `trial`; cancelling out of the hosted checkout returns to onboarding with progress intact.** The push permission prompt fires at the correct moment (after preferences, before plan generation). The 035-V forms are now backed by real persistence, completing the 035 pair.
...
> **DESIGN-TRACK RE-OVERHAUL RE-SCOPE — see Addendum → "Design-Track Re-Overhaul."** This not-yet-run `-V` half runs on the **single build model** (the Fable carve-out is retired) and targets the **rich / animated / immersive** posture built from code libraries (Lenis, GSAP, Vanta + Three.js, react-bits). The "first-plan cinematic loading + reveal" is **restored as a rich cinematic loading + reveal** (candle-flame/ember loader + immersive sequential-reveal choreography via GSAP / react-bits; cinematic/Considered-band tokens; **reduced-motion → instant** mandatory); the title "cinematic" and the cinematic wording below are active direction again. The AI-generated-imagery / video GENERATION pipeline stays retired (richness from code libraries, not generated art). The build model authors the styled component code directly (Option 1). 106/107/107a token + primitive values are unchanged.
...
**Goal:** Build the static, design-locked first-plan experience against a mock plan: the streaming-skeleton loading screen (shimmer placeholders with voice-gated loading copy), the cinematic reveal of the first plan, the feature tour (four to five swipeable screens covering mark complete, NL input, Dynamic Island, weekly planner; all skippable at any point), and — **new under the direct-release pivot** — the **dismissable "get the iOS app" overlay** that is the last thing a web user sees before the app itself. The real plan-generation trigger and SSE streaming are deferred to 036-W; this half uses a canned mock plan to build the loading/reveal/tour choreography.

**The App Store overlay (direct-release pivot).** With no waitlist there is no launch-day email to carry the App Store link, so the flow's own terminal has to carry it: after the tour, a **web-only, dismissable** overlay offers the iOS app and, on dismiss, the user lands on the signed-in web app home (`/plan`). It is **dismissable by construction** — a visible dismiss affordance, dismissable by escape/backdrop, never blocking, never a second onboarding step — and it is **not a second source of truth on availability**: its copy is the same "iOS first, Android coming later" posture the 093-V landing states and the StoreKit chats (085/086) assume, authored here through the caveman/stop-slop gate. **Determination:** this half owns the overlay because 036 already owns the tour-to-day-view terminal of onboarding, which is exactly where the overlay sits; the 032 chats own screens 1–3 (too early) and the 085/086 chats own native iOS StoreKit (wrong surface — the overlay is a web surface pointing *at* iOS). No new chat entry was created for it.
...
- The **dismissable "get the iOS app" overlay** (web only), presented after the tour: the App Store affordance, the voice-gated copy in the "iOS first. Android coming later." posture, and a plain dismiss affordance. Non-blocking; escape and backdrop dismiss; no second onboarding step and no manufactured urgency. Presentation only — the dismiss-to-`/plan` navigation is wired in 036-W
...
**End-of-session checks:** The generating-skeleton screen animates calmly through the mock-plan sequence with voice-gated rotating copy on both web and mobile. The cinematic reveal plays against the mock plan. The tour renders four to five swipeable screens with skip available, all copy cleared through the voice gate. **The App Store overlay renders on web after the tour, is dismissable by its own affordance and by escape/backdrop, blocks nothing, and its copy matches the landing's availability posture.** No real plan-generation trigger, SSE wiring, or dismiss navigation is present (deferred to 036-W).
...
**Goal:** Rewire the 036-V loading/reveal/tour choreography to the real plan-generation pipeline: trigger the user's first plan generation at the end of onboarding by calling POST /plans/generate, stream the real SSE response into the skeleton, drive the reveal from the real chunks, show the feature tour after the plan completes, present the 036-V App Store overlay on web, and — on dismiss — land the user on the signed-in web app home. **Direct-release pivot:** that home is the day view at `/plan` (`apps/web/app/(app)/plan` [code], built by chat 039), and it is the **terminal of the whole direct-release flow** — landing CTA (093-V) → signup (032) → pricing + card capture (035) → App Store overlay (036) → `/plan`. Recording the terminal here is the entirety of 039/040's involvement in the pivot; neither chat's own scope changes.

**Output:**
- `apps/web/lib/firstPlan.ts` and the mobile equivalent — the orchestration: call POST /plans/generate (chat 025) with the user's onboarding-derived energy default of 5, stream the real response into the 036-V skeleton, navigate to the tour on completion, and on tour completion or skip present the App Store overlay on web (mobile skips it) and navigate to the day view at `/plan` on dismiss
- Dismiss wiring on the 036-V App Store overlay: dismissing (by affordance, escape, or backdrop) navigates to `/plan`; the overlay is shown once at the end of first onboarding and is never re-presented as a blocking step
...
**End-of-session checks:** A fresh user completes onboarding and sees a streaming first plan driven by the real SSE response. The plan completes successfully (using the real synthesizePlan from chat 022 via the chat-025 route). The tour renders correctly with skip available. **On web, the App Store overlay appears after the tour and dismissing it lands the user on `/plan`; the overlay never blocks and is not shown on mobile.** The user lands on the day view. The 036-V choreography is now backed by the real generation pipeline, completing the 036 pair and closing the Block 5 onboarding flow.
...
> **DESIGN-TRACK RE-OVERHAUL RE-SCOPE — see Addendum → "Design-Track Re-Overhaul."** This not-yet-run `-V` half runs on the **single build model** (the Fable carve-out is retired) and targets the **rich / animated / immersive** posture built from code libraries (Lenis, GSAP, Vanta + Three.js, react-bits): expressive motion, immersive staging, layered depth — **reduced-motion → instant** mandatory. The fitness and nutrition block-detail layouts here must still stay consistent with the **method-B module scaffolds** (ADD-C fitness, ADD-B nutrition): scaffold-breadth surfaces only; do not add detail chrome for the deferred deep engines (micronutrient/RDA/calorie; strength-rank/percentile) — the rich posture governs the *treatment*, not the scaffold's data breadth. The AI-generated-imagery / video GENERATION pipeline stays retired (richness from code libraries, not generated art). The build model authors the styled component code directly (Option 1). 106/107/107a token + primitive values are unchanged.
...
### Chat 092 — Resend Email Templates Part 2: Post-Cancel and Referral (waitlist emails RETIRED)

*Block 12 · EO 88 · 🎩 🟡 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*

**Load at session start:** LAYER_4_EXPERIENCE_IDENTITY.md (email copy library, post-cancel survey); LAYER_6_LAUNCH_GROWTH.md (Landing Page Strategy — the direct-release landing, post-pivot; Referral Program); chat 017 (voice gate); chat 091 (email rendering pattern established); the direct-release-pivot flag in `docs/Persistent.md`.

**Goal:** Author the remaining marketing-class React Email templates. **Direct-release pivot — two of the original three are retired:** the waitlist nurture email ("Building.") and the waitlist launch-day email (iOS / Android variants) **are not authored**, because there is no waitlist to nurture and no launch-day blast to send — visitors sign up from the landing on the day they arrive. What remains is the **post-cancel 48-hour survey email** that asks one open-ended question about what would have made Vesper work, plus the referral-email copy this chat already carried. The App Store link the retired launch-day emails would have carried is now delivered in-product by the dismissable App Store overlay at the end of onboarding (036-V / 036-W).

**Output:**
- ~~`packages/shared/emails/WaitlistNurture.tsx`~~ — **RETIRED (direct-release pivot): do not author**
- ~~`packages/shared/emails/WaitlistLaunchIos.tsx`~~ — **RETIRED (direct-release pivot): do not author**
- ~~`packages/shared/emails/WaitlistLaunchAndroid.tsx`~~ — **RETIRED (direct-release pivot): do not author**
...
**End-of-session checks:** Every template this chat authors renders correctly. **No waitlist template is authored** (nurture and both launch-day variants are retired by the direct-release pivot). The post-cancel survey copy is gentle and not pushy.

### Chat 093-V — Product Landing Page with Three.js Cinematic (Visual / Static)

*Block 12 · EO 15 · 🔵 🎩 · Skills: caveman, stop-slop · ⚠️ · Window Y (Fwin F4) · CD-flags: —*

> **DESIGN-TRACK RE-OVERHAUL RE-SCOPE — see Addendum → "Design-Track Re-Overhaul."** This not-yet-run `-V` half runs on the **single build model** (the Fable carve-out is retired) and targets the **rich / animated / immersive** posture. **The Three.js / React Three Fiber immersive-scroll hero is RESTORED** — build a rich, animated, immersive marketing hero from the code-library tooling: **Vanta + Three.js** WebGL staging, **GSAP** ScrollTrigger + **Lenis** smooth-scroll choreography, **react-bits** animated components, composing the 107 design tokens, plus the five locked sections. The cream→espresso gradient + wordmark stays the **reduced-motion → instant fallback** (mandatory), not the baseline. **The AI-generated-imagery / video GENERATION pipeline stays retired** — no Google-Flow stills, no generated manor art; the richness is code-driven WebGL/motion, and the WebGL scene is authored in-code (low-poly / procedural), not from generated glTF art. The Three.js mobile-performance consideration returns with the immersive hero (perf-budget + reduced-motion path required); the ⚠️ flag reflects both that and "highest-stakes first-impression surface." **Direct-release pivot:** there is no waitlist and no `-W` binding — section 5's form is replaced by a signup CTA routing to the auth entry, and 093-W is retired (see its entry below). The build model authors the styled component code directly (Option 1). 106/107/107a token + primitive values are unchanged. **Recon note:** 093-V has **not** shipped (marketing page is still the `<h1>Vesper</h1>` stub; no `HeroThreeScene` yet), so this is a clean forward build; the code-library tooling (three/gsap/lenis/vanta/react-bits) is installed by the Design-Track Re-Overhaul chat — see Addendum → "Design-Track Re-Overhaul" → tooling + "Landed-work status."

**Load at session start:** LAYER_4_EXPERIENCE_IDENTITY.md (Marketing Visual Language, landing page composition); LAYER_6_LAUNCH_GROWTH.md (Landing Page Strategy — the direct-release landing, post-pivot); DESIGN_STRATEGY.md §4.1 (landing surface design note); OPEN_SOURCE_INVENTORY.md (Three.js, R3F); chats 107/107a (the design system and component library these surfaces compose from); chat 014 (CSP configuration / precompiled-shader strategy); chats 010 / 032-V (the auth entry the CTA routes into).

**Goal:** Build the **launched product's public front page** at `apps/web/app/(marketing)/page.tsx` with five sections in the Layer 4 composition — hero with the Three.js cinematic scroll experience, "What Vesper does," modules, "How it works," and pricing plus signup — as a static, design-locked surface running on **canned templates with no account**. **Vesper ships direct-to-release: there is no waitlist.** Section 5 carries **no email capture and no segmented control**; it carries the pricing literal ("One week free. After that, $19.99 per month." — unchanged) and a single primary **signup CTA that routes to the auth entry** (`apps/web/app/(auth)/sign-in/page.tsx` today [code]; repositioned into the onboarding flow as `(onboarding)/sign-in` by 032-V). The CTA is a plain route link — no form, no POST, no client-side capture state. The page is fully responsive; performance on mobile devices is the constraint that drives every implementation decision because the target audience checks the link from their phone. As a design-cluster-window chat (Fwin F4) it depends only on already-built foundations plus the design system per the Part 1 window note, and runs while the build track builds the backend spine in parallel [doc:PHASE_4_BUILD_PLAN_part1.md].
...
- `apps/web/components/marketing/PricingAndSignup.tsx` — the fifth section with the $19.99 monthly subscription price, the unchanged pricing literal ("One week free. After that, $19.99 per month."), the **card-at-signup disclosure** (the card is placed at signup and the first charge falls at trial end unless cancelled — stated plainly, not buried), and a **single primary signup CTA routing to the auth entry**. No email input, no iOS/Android segmented control, no submit handler, no POST. The platform-availability line ("iOS first. Android coming later.") stays as a voice-gated disclosure adjacent to the CTA, since Android visitors still need it before they start — but it is informational copy now, not a form control. The final CTA label and the disclosure wording are authored in this chat through the caveman/stop-slop gate
...
**Implementation notes:** This is the highest-stakes web surface in Phase 4 because the landing page is the application's first impression, which is why the prior plan marked it ⚠️. **The 🎩 flag is propagated here:** the hero copy, the section copy, the "iOS first. Android coming later." disclosure, the card-at-signup disclosure, and the CTA label are user-facing butler copy cleared through the caveman/stop-slop voice gate in this chat. **This chat now owns the whole 093 surface** — the direct-release pivot removed the waitlist POST, so there is no wiring half and 093-W is retired; there is no confirmation state to author, because the CTA navigates rather than submitting. Three.js performance on mobile is the technical risk: an unoptimized scene takes seconds to load on a mid-range Android phone, which destroys conversion; optimization tactics include compressed glTF (Draco), baked lighting, instanced meshes, and progressive scene complexity by detected device capability. Shaders are precompiled at build time via `vite-plugin-glsl` and loaded as static assets so the production CSP can omit `'unsafe-eval'` (cross-reference Decision 13 from chat 001 — the CSP omission and the precompiled-shader strategy are the same architectural decision viewed from two angles). **Risk note:** 093-V is a design-track window chat (EO 15) and retains the ⚠️ flag for the Three.js mobile-performance risk concentrated in this visual half; elevated-risk handling applies. **Window vs CD-flag scoping:** `Fwin F4` is the fourth window-chat position and is unrelated to any `CD-flag F#`; this chat carries no CD-flag, and the `Fwin#` and `CD-flags` fields are read field-scoped per the Part 1 naming caveat [doc:PHASE_4_BUILD_PLAN_part1.md].

**Dependencies:** Chats 107a (component library) and 014 (CSP / precompiled-shader configuration), plus the committed LAYER_4 marketing copy library; per the Part 1 window note the chat depends only on already-built foundations plus the design system earlier in the same cluster and on **no unbuilt build-track chat** [doc:PHASE_4_BUILD_PLAN_part1.md][doc:PHASE_4_REPLAN_STAGE_2_REORDER.md]. The CTA target is the **built** chat-010 auth surface (`apps/web/app/(auth)/sign-in/page.tsx` [code]), so the link resolves at build time; 032-V later repositions that surface inside the onboarding flow and 093-V's CTA follows it. It no longer depends on the chat-031 waitlist API in any form — 031's waitlist half is retired by the direct-release pivot. Runs at EO 15 inside the window; no wiring half, no later-EO dependency.

**End-of-session checks:** The page renders correctly on desktop, tablet, and mobile with the right voice-gated copy, composed from 107a primitives. Lighthouse mobile performance score is 90 or higher. The Three.js scene runs at 60fps on an iPhone 12 (mid-range target); the reduced-motion / no-WebGL fallback renders. Section 5 renders the pricing literal, the card-at-signup disclosure, and a single signup CTA that navigates to the auth entry — with **no email input, no segmented control, and no network call anywhere on the page**. The 093 surface is complete on its own (no wiring half).

### Chat 093-W — RETIRED (was: Waitlist Signup POST Wiring)

*Block 12 · EO — (never in master table) · 🔵 · Skills: — · — · Window N · CD-flags: —*

**RETIRED — do not run.** This chat existed for one reason: to bind the 093-V signup form's submit handler to `POST /waitlist` (chat 031). The **direct-release pivot retires the waitlist entirely** — Vesper ships direct-to-release, the landing at `apps/web/app/(marketing)/page.tsx` is the launched product's front page, and section 5 carries a **signup CTA that is a plain route link to the auth entry**, not a form. A route link has no submit handler, no request body, no success state and no error state, so **there is nothing left to wire**: 093-W is retired rather than repurposed, and 093-V now owns the whole 093 surface single-handed.

**Determination (recorded, not inferred):** retire, not repurpose. The alternative — repurposing 093-W to "wire the landing CTA to the auth/signup entry" — was rejected because the CTA is a `Link` to an already-built route (`apps/web/app/(auth)/sign-in/page.tsx`, chat 010 [code]); authoring one `href` is not a chat's worth of scope, and 093-V already owns section 5's markup and its voice-gated copy.

**Master-table impact: none.** 093-W never had a row in the Part 1 master reordered-sequence table or in the Stage 2 §4 full reordered sequence [doc:PHASE_4_BUILD_PLAN_part1.md][doc:PHASE_4_REPLAN_STAGE_2_REORDER.md] — its existence came only from Stage 2 §3's `-V`/`-W` split of 093 and Stage 2 §5's "all `-W` halves run post-window on the build track." Retiring it therefore removes no EO and changes no ordering; the pre-existing master-table-vs-Stage-2 discrepancy it used to surface is now moot for 093.

**Consequence for the 093 pair:** the "a `-V` half is not done until its `-W` half ships" rule does not bind 093. 093-V ships complete on its own.
...
After all chats and the Cutover steps complete, the application is submitted to App Store Review and the product landing page is live on the production domain. Phase 5 covers launch coordination, App Store approval response, and the first month of operation.
...
### Chat ADD-D — Rich Polish Pass on Shipped Design Surfaces (Addendum / Corrective)

*Addendum · no EO (runs immediately, out of sequence) · **runs on the single build model** (Fable carve-out retired) · 🔵 🟢 🟡 · Skills: frontend-design · — · Window N · CD-flags: —*

**SUPERSEDES:** nothing structurally — this chat **lifts already-shipped design surfaces to the rich / animated / immersive finished-product bar** authored directly on the single build model. It supersedes the assumption that the shipped 107/107a primitives and the shipped plan/day/week/settings surfaces stay at their as-built visual quality; they are re-styled to the rich bar. It also supersedes the earlier "clean-posture / Fable-model" framing of this same chat (that framing is retired with the re-overhaul). It does **not** supersede the token **contract** (see the loosened token-freeze rule below).

**SOURCE OF TRUTH:** the **loosened token-freeze contract** in the "Design-Track Re-Overhaul" section below (existing token names + values frozen; NEW motion/immersive token names allowed; primitive *styling* + motion is build-model-revisable) governs what this chat may and may not change.

**Load at session start:** the shipped **107/107a** system — `packages/ui/src/{tokens.ts, tailwind.ts, index.ts}`, the core primitives (`Card`, `Button`, `BlockRow`, `ButlerLine`) and part-2 primitives (`TextField`, `Toggle`, `Select`, `SegmentedControl`, `TimePicker`, `DatePicker`, `ButlerVoice`, `motion`) in `apps/web/components/ui` + `apps/mobile/components/ui`, and `docs/DESIGN_SYSTEM.md`; the shipped **plan/day** components (`apps/web/components/plan/*`, `apps/mobile/components/plan/*` — `BlockCard`, `BlockTimeline`, `PlanSkeleton`, `PlanEmpty`), the **week** page (`apps/web/app/(app)/week/page.tsx`), and the **settings** surfaces (`apps/web/app/(app)/settings/*`); `docs/DESIGN_STRATEGY.md` (rich-posture banner); the "Design-Track Re-Overhaul" section below (loosened token-freeze contract + tooling + priority order).

**Goal:** Run a **visual polish pass over the design surfaces that have ALREADY SHIPPED**, lifting them from as-built quality to the rich, animated, immersive **finished-product (resume / YC-demo) bar** — the single build model writes the styled component code directly (Option 1), composing the code-library tooling (Lenis, GSAP, Vanta + Three.js, react-bits) where it adds value. Scope is **visual treatment only**: the primitive components' look-and-feel and the shipped plan/day/week/settings surfaces' composition, spacing, and motion. **Token NAMES stay stable and existing VALUES are unchanged** (see the loosened token-freeze contract) so every existing consumer (BlockCard, BlockTimeline, subscription UI, calendar chrome) stays valid; NEW motion/immersive tokens may be added by NAME. **Reduced-motion → instant** is mandatory on every motion surface.

**Output:**
- Re-styled core + part-2 primitives in `apps/web/components/ui` and `apps/mobile/components/ui` to the rich bar — same component APIs, same imported token **names**; NEW motion/immersive tokens may be added by NAME to `tokens.ts` (mirrored in `tailwind.ts` / `DesignTokens.swift`), never by inlining a hex/px/ms
- Re-styled shipped plan/day surfaces: `BlockCard`, `BlockTimeline`, `PlanSkeleton`, `PlanEmpty` (web + mobile), the `week` page, and the settings surfaces — spacing, hierarchy, motion, and treatment lifted to the rich bar, composing the re-styled primitives
- The **`cinematic` motion band/easing** token is **KEPT** (do NOT remove it — the earlier "remove cinematic" option is cancelled; rich motion is back and the band is in use again)
...
**Implementation notes:** This chat is the mechanism for the premise "**the build model authors the finished rich designs itself**, from code libraries, no AI-gen imagery / video pipeline." It runs on the **single build model** (the Fable / ~$100-budget carve-out is retired) and is placed in the **priority order** below (it targets the highest-visibility shipped surfaces: the design system every screen inherits, and the plan/day view a demo spends the most time on). The **loosened token-freeze contract** is the safety rail: because BlockCard/BlockTimeline/subscription-UI/calendar-chrome all read `@vesper/ui` token **names**, mutating an existing token **value** would silently re-skin consumers the pass did not review — so existing names + values stay stable and only primitive *styling* + surface composition + motion change, with NEW motion/immersive tokens added by NAME. Rich motion/immersive treatment **is** added (GSAP/Lenis/Vanta+Three.js/react-bits where they add value), each with a **reduced-motion → instant** path; this is a rich-finish pass. The AI-image/video GENERATION pipeline is **not** resurrected. Native Swift surfaces (059b / 077 / 078) stay build-track-owned from the 107 spec (unchanged). If the pass finds a surface that genuinely needs a new token value, it adds the token (new name) rather than mutating an existing value, keeping every current consumer stable.

**Dependencies:** Already-built foundations only — the shipped 107/107a system and the shipped plan/day/week/settings surfaces named above; **ADD-A** where the pass reaches Modules-tab surfaces (so it styles the post-refactor navigation, not the superseded one). No dependency on any unbuilt master-table chat. The unbuilt `-V` halves are **not** in this chat's scope — they are authored fresh on the single build model when they run; this chat only polishes what already shipped.

**End-of-session checks:** The shipped primitives and the plan/day/week/settings surfaces render at the rich bar and still compose from `@vesper/ui`. **No existing token NAME or VALUE changed** (verify `tokens.ts` diff is additive only — new token names allowed; `cinematic` retained); every existing consumer still builds. Every new motion surface has a **reduced-motion → instant** path. `pnpm build` / `pnpm lint` clean; primitive unit tests green. No AI-generated-image/video element was introduced (rich motion is code-driven). `DESIGN_SYSTEM.md` reflects the styling + motion changes.

---

## Design-Track Re-Overhaul — Rich Posture + Single Build Model (doc-level re-scope; not a chat)

This is a **direction re-scope**, not a chat: it **reverses the earlier "Clean Posture + Fable Model" re-scope** — returning the design track's posture to **rich / animated / immersive** and its execution to the **single build model** (retiring the Fable carve-out), editing the strategy/direction docs, adding the code-library tooling, and re-scoping the not-yet-run `-V` visual halves. It carries **no ID and no EO**.

### Posture change (return to the rich / immersive vision)

The design track **returns to the rich, animated, immersive vision**, built from **code libraries** (Lenis smooth-scroll, GSAP, Vanta + Three.js WebGL, react-bits) rather than an AI-image/video generation pipeline. The earlier clean-posture drop was made because the AI-image/video 3D-**generation** pipeline (Google Flow) was too slow — that GENERATION pipeline **stays retired**; what returns is rich/immersive UI built from code libraries that plug into the build model. The target is a **rich, animated, immersive modern-app experience**: expressive motion, immersive staging, layered depth — restrained-clean is no longer the ceiling. **Reduced-motion → instant** is preserved on every motion surface. The product must still read as a finished product (resume / YC-demo bar). Loss-aversion/persuasion ethics and the scorekeeping-as-absence discipline from `DESIGN_STRATEGY.md` §2–3 are unchanged.

### Tooling (added by the Design-Track Re-Overhaul chat)

The code-library tooling that makes the rich posture buildable is added to the workspace: **lenis** (smooth-scroll), **gsap** (timeline / ScrollTrigger motion), **vanta** + **three** (WebGL animated backgrounds; three is vanta's peer), **react-bits** (animated React components), and **Playwright** (visual / interaction checks — already a dev dep). No CSP directive was needed: the installed libraries (three-core, vanta, gsap, lenis) were audited and carry **zero** `eval` / `new Function`, and WebGL shader compilation is GPU-side (not JS-eval), so `script-src` was left unchanged and Decision 13's `'unsafe-eval'` exclusion stays fully intact (audit documented in `docs/CSP_NOTES.md`). Claude Code SKILLS (taste, web-design-guidelines, image-to-code) and `npx playwright install` are **user-installed prereqs** — not agent-installable.
...
- **Chat 106 output** `docs/DESIGN_STRATEGY.md` — kept; **amended** with the rich-posture banner + the §4.1 hero line restored to the immersive direction (see below). No persuasion/scorekeeping decision changed.
- **Chat 107 output** `docs/DESIGN_SYSTEM.md` + the committed `@vesper/ui` token layer (`packages/ui/src/tokens.ts`, `tailwind.ts`) + core primitives (`Card`, `Button`, `BlockRow`, `ButlerLine`) — **untouched.** Token **values** (including the `cinematic` motion band and `cinematic` easing) are **not changed** — changing them would invalidate landed work. Under the rich posture the `cinematic` band is **in use again for rich / immersive motion**; it is **KEPT** (the earlier option to remove it is cancelled). (DESIGN_SYSTEM.md is updated by ADD-D to record the rich baseline — see ADD-D — but no token value changes.)
...
**RE-ACTIVATED vs STILL-DEPRECATED split under the rich re-overhaul (content preserved, marked, pointed to this section):**
- `docs/VISUAL_DIRECTION_BANK_v2.md` — the immersive motion/staging bank (Three.js/R3F, GSAP+Lenis scroll, WebGL scenes, reel-derived staging technique). **RE-ACTIVATED for its motion + immersive-staging direction** — it is active direction again, executed on the single build model with reduced-motion → instant mandatory. **Its AI-generated-imagery parts stay retired** (no AI-atmospheric stills / generated manor art): consume the technique / motion / staging, translate any AI-image slot to code-driven WebGL/CSS staging.
- `docs/ASSET_MANIFEST_KICKOFF.md` — the AI-image generation kickoff (operator generation playbook for AI-generated manor stills / atmospheric imagery). **STAYS deprecated-superseded** — the rich re-overhaul does **not** resurrect the AI-image/video GENERATION pipeline.
- `docs/REEL_REFERENCE_SHEETS/` (REEL2/6/7/8/9 `.jpg` + `README.txt`) — AI-image mood/staging reference stills. **STAY deprecated-superseded** (marked via the folder `README.txt`; images preserved) — the AI-image GENERATION pipeline is not resurrected.

**No rich/immersive code has shipped yet — this is a clean forward build.** Recon (filesystem + deps) found: the immersive direction lived only in the **direction docs** and in **not-yet-run `-V` plan Goals** — no immersive UI code landed (the `(marketing)/page.tsx` is still the bare `<h1>Vesper</h1>` stub; `HeroThreeScene` is absent; the onboarding, block-detail, subscription, and referral `-V` surfaces are unbuilt). The rich-UI tooling (`lenis`, `gsap`, `vanta` + `three`, `react-bits`) is **installed by this re-overhaul chat**; `vite-plugin-glsl` is not needed (WebGL scenes are authored in-code, not from a shader/asset pipeline). The **only** shipped design code is the **107/107a system** + the build-track plan/day surfaces that compose from it. That shipped code is not *wrong*; it is at as-built quality, and is **lifted to the rich bar by ADD-D (single build model)** under the loosened token-freeze contract below. The unbuilt `-V` halves are authored fresh on the single build model when they run.

### Loosened token-freeze contract (the safety rail for touching shipped design code)

The build model **may** revise, on the shipped surfaces (via **ADD-D**): the **visual styling** of the 107/107a primitives, the composition/spacing/hierarchy/motion of the shipped plan/day/week/settings surfaces — to the rich bar — and **may add NEW motion/immersive tokens by NAME**.

The build model **may NOT**: change any **existing token name or value** in `packages/ui/src/tokens.ts` (or its `tailwind.ts` mapping / `DesignTokens.swift` mirror), or inline a hex/px/ms into a component. A new value enters only as a **new token NAME** under the "add-a-token, never inline" rule — never by mutating an existing value. **Reason:** BlockCard, BlockTimeline, the subscription UI, and the calendar chrome all read `@vesper/ui` token **names**; mutating a value would silently re-skin consumers the pass did not review. So: **existing token names + values = frozen contract; NEW motion/immersive token names = allowed; primitive *treatment* + surface composition + motion = revisable.** The `cinematic` band/easing token is **KEPT** (the earlier "remove cinematic" option is **cancelled** — rich motion is back and the band is in use). Every new motion surface carries a **reduced-motion → instant** path.

### Single build model (Fable carve-out RETIRED)

Design-track chats (the `-V` visual halves going forward) **run on the single build model** — the earlier Fable-model / ~$100-budget carve-out is **retired**. There is no separate design-track model or budget: the build model authors the rich styled component code directly (Option 1 — no Figma, no image pipeline), composing the code-library tooling (lenis, gsap, vanta + three, react-bits). This restores the Part 1 build-plan "single model" rule for the design track; the standing note in `CLAUDE.md` is updated to match.

### Re-scoped `-V` halves (all not-yet-run; each carries an in-place RE-SCOPE pointer above)

Eleven `-V` visual halves are re-scoped to the **rich / immersive + single-build-model + new-surface** posture (in-place pointers updated at each): **032-V, 033-V, 034-V, 035-V, 036-V, 041-V, 044-V, 046-V, 089-V, 093-V, 095-V.** Notable retargets: **035-V** and **041-V** still target the **new Modules-tab + method-B scaffold surfaces** (ADD-A/B/C), not the superseded settings-tab module mounts; **036-V** restores the **rich cinematic** loading/reveal; **093-V** restores the **Three.js/R3F immersive-scroll hero** built from the code-library tooling. (The design-foundation chats 106/107/107a are **not** re-scoped — they are landed and kept.)

### Priority order (single build model — high-visibility first)

The rich design pass is applied in this **priority order** so the **YC-demo-critical, high-visibility** surfaces land first. Both the not-yet-run `-V` halves and **ADD-D** (the polish of already-shipped surfaces) share this order:

0. **ADD-D — shipped design system + plan/day surfaces** (107/107a primitives, BlockCard/BlockTimeline/PlanSkeleton/week/settings). Placed first because the primitives are inherited by *every* later surface (`-V` halves compose them), so lifting them to the rich bar raises the floor everywhere at once; the shipped plan/day view is also the surface a demo spends the most time on. (Existing token values stay frozen — see the loosened token-freeze contract.)
...
4. **Product + referral landing** (093-V immersive hero, 095-V referral landing) — external-facing, lower demo-loop priority than the in-app core.
5. **Subscription lifecycle** (089-V) and the remaining `-V` halves (033-V, 034-V, 044-V) — refine as time allows.

Surfaces below the cut line still ship: they compose from the committed 107/107a system (lifted to the rich bar by ADD-D), so they read as finished even before a dedicated rich pass — the rich pass is polish on top of a finished baseline, not the baseline itself.

### Landed-work status (resolved by recon)

**Recon result: no immersive code had shipped, so this is a clean forward build with no removal follow-up.** Filesystem recon confirms `apps/web/app/(marketing)/page.tsx` is still the bare `<h1>Vesper</h1>` stub and `HeroThreeScene` does not exist; the onboarding, block-detail, subscription, and referral `-V` surfaces are unbuilt. The rich-UI tooling (`lenis`, `gsap`, `vanta` + `three`, `react-bits`) is **installed by this re-overhaul chat** (`vite-plugin-glsl` is not needed — WebGL scenes are authored in-code). **093-V's Three.js immersive hero is a forward build** — its re-scope is covered by editing its Goal-pointer in-place (done above). The only shipped design code is the **107/107a system** + the build-track plan/day surfaces that compose from it; those are lifted to the rich bar by **ADD-D** (single build model) under the loosened token-freeze contract.
