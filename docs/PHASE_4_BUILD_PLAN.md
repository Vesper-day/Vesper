...
> **DESIGN-TRACK RE-OVERHAUL RE-SCOPE — see Addendum → "Design-Track Re-Overhaul."** This not-yet-run `-V` half runs on the **single build model** (the Fable carve-out is retired) and targets the **rich / animated / immersive** posture. **The Three.js / React Three Fiber immersive-scroll hero is RESTORED** — build a rich, animated, immersive marketing hero from the code-library tooling: **Vanta + Three.js** WebGL staging, **GSAP** ScrollTrigger + **Lenis** smooth-scroll choreography, **react-bits** animated components, composing the 107 design tokens, plus the five locked sections. The cream→espresso gradient + wordmark stays the **reduced-motion → instant fallback** (mandatory), not the baseline. **The AI-generated-imagery / video GENERATION pipeline stays retired** — no Google-Flow stills, no generated manor art; the richness is code-driven WebGL/motion, and the WebGL scene is authored in-code (low-poly / procedural), not from generated glTF art. The Three.js mobile-performance consideration returns with the immersive hero (perf-budget + reduced-motion path required); the ⚠️ flag reflects both that and "highest-stakes first-impression surface." The `-W` binding to POST /waitlist is unchanged. The build model authors the styled component code directly (Option 1). 106/107/107a token + primitive values are unchanged. **Recon note:** 093-V has **not** shipped (marketing page is still the `<h1>Vesper</h1>` stub; no `HeroThreeScene` yet), so this is a clean forward build; the code-library tooling (three/gsap/lenis/vanta/react-bits) is installed by the Design-Track Re-Overhaul chat — see Addendum → "Design-Track Re-Overhaul" → tooling + "Landed-work status."
...
4. **Waitlist + referral landing** (093-V immersive hero, 095-V referral landing) — external-facing, lower demo-loop priority than the in-app core.
