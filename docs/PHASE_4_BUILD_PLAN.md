...
**EO is a dependency rank, not a schedule.** A position number is the chat's place in a single dependency-valid topological walk; it is not a wall-clock day and there are no day bands in this document. The build runs on **two parallel tracks** — the **F (Fable) design track** and the **B (Opus/Sonnet) build track** — across the same calendar. A high EO does not mean "starts later in wall-clock" for a build-track chat whose prerequisites are already built; it means later in the topological order. The two tracks interleave in the single EO sequence but run concurrently.

**The Fable window.** Fifteen chats (marked `Y` in the Window column) must finish inside the ≤11-day Fable window. Those fifteen also carry a Fable-window sequence counter, `Fwin F1`–`F15`, so window load is visible at a glance; the counter is independent of the build-track EO positions interleaved around it. The window note directly under the master table records the feasibility result, the designated pressure valve, and a counter-vs-flag naming caveat.

**Splits.** Thirteen chats are split into a Fable visual/static half (`NNN-V`, inside the window) and an Opus/Sonnet wiring half (`NNN-W`, in normal sequence). **Both halves are documented inside the original chat's Block** — the split is a sequencing and ownership device, not a relocation. Any existing reference to `NNN` resolves to the pair. The `-V` halves are design-locked, wiring-deferred: durable visual output that the `-W` half rewires to real APIs after Fable leaves. A `-V` half is not "done" until its `-W` half ships.

**New chats.** Seven new chats carry fresh IDs ≥106 so they cannot collide with any existing number (including the intentionally-deleted 068–070, left untouched). The three pure design-foundation chats (106 design-strategy, 107 design system, 107a component library part 2) are documented in a new **Design Cluster** section placed ahead of Block 0. The Butler's Notebook pair (108 migration/API on the build track, 108a visual card on the Fable track) is documented in a new **Butler's Notebook** section adjacent to the Design Cluster. The two stale-code fix chats are documented in the blocks they repair: 111 (schema/Zod fixes and the single early migration-read checkpoint) in Block 1, and 112 (AI fallback re-scope and chat-019 prompt audit) in Block 3.

Risk-flagged chats (⚠️) require special handling: plan mode on, the assigned model held at Opus regardless of any Sonnet/Haiku note, multiple iterations if needed, no shortcuts. The Risk Map explains why each flagged chat carries elevated risk. Parallel execution across terminals is encouraged where chats are marked ⇄; the Parallelization Map enumerates concurrency.
...
**Model** (new in the re-plan) identifies the model assigned to the session. **Fable** marks a Fable design-track session (visual systems, static/mocked surfaces, the landing demo); every Fable chat is on the **F track**. **Opus** is the default build-track model and is left implicit when no other build model is named. **Sonnet** and **Haiku** mark build-track sessions the plan specifically assigns to those models. **Opus-native** marks Opus build-track sessions that target SwiftUI / WidgetKit / ActivityKit native code (the alarm screen and Live Activity surfaces); Fable does not own native Swift even when the work is low-dependency. All non-Fable models are on the **B track**. Risk-flagged (⚠️) chats are held at Opus regardless of any lighter assignment.
...
*Block <N> · EO <##> · Model <Fable | Opus | Sonnet | Haiku | Opus-native> · <🔵|🟢|🟣|🟡> · Skills: <list or —> · <⚠️ | 🚧 | ⇄ + partner IDs, or —> · Window <Y (Fwin F#) | N> · CD-flags: <F1..F4 | —>*
...
Field-mapping notes for authors of later parts: **scope → Goal**; **prerequisites → Load at session start (context) + Dependencies (gating IDs)**; **outputs → Output**; **notes → Implementation notes + End-of-session checks**. The meta line is where **EO** and **Model** are surfaced per entry; they must agree with the master table. A split half uses the same template with its `-V`/`-W` ID and its own EO, model, and window values. New chats use the same template.

---

## Master Reordered-Sequence Table

Every chat, existing and new, in execution order. **Track** is `F` (Fable) or `B` (build). **Model** is the assigned model. **Win** is the Fable-window membership; **Fwin#** is the in-window sequence counter (window chats only). **CD-flags** lists carried-forward CANNOT-DETERMINE flags the chat reads (R) or consumes (C). The deferred V1.5 chat 090a is not in the order. This table — not the chat numbers, not the block order — is the execution order of the build.

| EO | Chat ID | Track | Model | Win | Fwin# | Scope (one line) | CD-flags |
|---|---|---|---|---|---|---|---|
| 1 | 106 | F | Fable | Y | F1 | Design-strategy doc (no code); honest-conversion map across app + site | — |
| 2 | 107 | F | Fable | Y | F2 | Design system (tokens) + core primitives; emits native token spec | — |
| 3 | 107a | F | Fable | Y | F3 | Component library pt 2 (forms, pickers, butler-line, motion) | — |
| 4 | 111 | B | Opus | N | — | Schema/Zod stale-fixes + single early migration-read checkpoint | F1 R, F2 R, F3 R |
| 5 | 112 | B | Opus | N | — | AI fallback re-scope (018) + chat-019 prompt audit | F4 R |
| 6 | 021 | B | Opus | N | — | Context builders + cache wiring | — |
| 7 | 023 | B | Opus | N | — | Other AI op scaffolds (NL parse, classify, check-in, regen) | — |
| 8 | 024 | B | Opus | N | — | Profile + energy APIs (uses 111-corrected profile shape) | — |
| 9 | 031 | B | Opus | N | — | Waitlist + referral APIs | F3 C |
| 10 | 047 | B | Opus | N | — | Seed sourcing scripts (ExerciseDB + TheMealDB) | — |
| 11 | 091 | B | Opus | N | — | Resend templates 1 (auth + trial reminders) | — |
| 12 | 108 | B | Opus | N | — | Notebook migration + table + RLS + read/confirm-correct API | — |
| 13 | 059b | B | Opus-native | N | — | iOS alarm screen (Opus owns; Fable visual spec via 107) | — |
| 14 | 077 | B | Opus-native | N | — | LA widget target + app group (Opus owns) | — |
| 15 | 093-V | F | Fable | Y | F4 | Landing page + Three.js hero demo (canned templates, no account) | — |
| 16 | 032-V | F | Fable | Y | F5 | Onboarding welcome / auth-reposition / honorific (visual + reveal) | — |
| 17 | 033-V | F | Fable | Y | F6 | Archetype tiles (visual) | — |
| 18 | 034-V | F | Fable | Y | F7 | Calendar-connect / walkthrough / location screens (static) | — |
| 19 | 035-V | F | Fable | Y | F8 | Module toggles + per-module prefs (forms, mocked; reflects 111 notif-prefs) | — |
| 20 | 036-V | F | Fable | Y | F9 | First-plan cinematic loading + reveal + tour (mock plan) | — |
| 21 | 041-V | F | Fable | Y | F10 | 10 block-type detail layouts (static, BlockDetailsSchema) | — |
| 22 | 044-V | F | Fable | Y | F11 | Ambient butler line (visual + local rotation, static library) — **pressure valve** | — |
| 23 | 046-V | F | Fable | Y | F12 | Vesper-hour static + week view + morning brief + energy slider + quiet hours | — |
| 24 | 108a | F | Fable | Y | F13 | Notebook visual card surface | — |
| 25 | 089-V | F | Fable | Y | F14 | Subscription lifecycle static states (trial-end ledger, banners, billing) | — |
| 26 | 095-V | F | Fable | Y | F15 | Referral landing `/r/[code]` page (visual) | — |
| 27 | 022 | B | Opus | N | — | synthesizePlan + fallback chain (uses 112-audited prompt) | — |
| 28 | 048 | B | Opus | N | — | Seed migrations 14–15 + first seed deploy | — |
| 29 | 026 | B | Opus | N | — | Plan retrieval APIs | — |
| 30 | 028 | B | Opus | N | — | Task APIs | — |
| 31 | 030 | B | Opus | N | — | Subscription / account / push-token scaffolds | — |
| 32 | 029 | B | Opus | N | — | NL command + weekly-priorities APIs | — |
| 33 | 078 | B | Opus-native | N | — | LA widget UI (3 variants + DesignTokens.swift from 107 spec) | — |
| 34 | 025 | B | Opus | N | — | Plan-gen API (SSE, idempotency, atomic commit) | F1 C |
| 35 | 027 | B | Opus | N | — | Block APIs (optimistic concurrency) | — |
| 36 | 037 | B | Opus | N | — | Supabase Realtime client | — |
| 37 | 063 | B | Opus | N | — | GCal OAuth + pgsodium + rotation runbook | — |
| 38 | 081 | B | Opus | N | — | Subscription state machine | — |
| 39 | 076 | B | Opus | N | — | Push token infrastructure (APNs + push-to-start) | — |
| 40 | 090b | B | Opus | N | — | Biometric lock setting (OFF default) | — |
| 41 | 052-W | B | Opus | N | — | Built-in calendar web (chrome + library + CRUD + RRULE; 052-V folded in) | — |
| 42 | 053 | B | Opus | N | — | Built-in calendar mobile (parity) | — |
| 43 | 054-W | B | Opus | N | — | Tasks: list UI + CRUD wiring (054-V folded in) | — |
| 44 | 064 | B | Haiku | N | — | GCal sync + token refresh + classification | — |
| 45 | 038 | B | Opus | N | — | Offline mutation queue + conflict resolution | — |
| 46 | 032-W | B | Opus | N | — | Onboarding state machine + auth/persistence wiring | — |
| 47 | 033-W | B | Opus | N | — | Archetype 5A/5B branch routing | — |
| 48 | 039 | B | Opus | N | — | Plan day view (web) — wire 037/038/026 | — |
| 49 | 040 | B | Opus | N | — | Plan day view (mobile) — parity | — |
| 50 | 041-W | B | Opus | N | — | Bind block-detail layouts to retrieved data | — |
| 51 | 042 | B | Opus | N | — | Block actions + state transitions + butler copy | — |
| 52 | 034-W | B | Opus | N | — | Onboarding calendar-branch + geolocation wiring (CP 063→064→034) | — |
| 53 | 035-W | B | Opus | N | — | Module-prefs persistence + push-prompt timing | — |
| 54 | 060 | B | Opus | N | — | Medications module (strict RLS, audit, local notifs) | F2 C |
| 55 | 061 | B | Opus | N | — | Finance + bills module (default OFF) | — |
| 56 | 057 | B | Opus | N | — | Weekly planning steps 1–3 | — |
| 57 | 085 | B | Opus | N | — | Apple StoreKit 2 (mobile purchase flow) | — |
| 58 | 083 | B | Opus | N | — | Stripe Checkout + Portal + cutover runbook | — |
| 59 | 044-W | B | Opus | N | — | Honorific-aware butler-line selection | — |
| 60 | 045 | B | Opus | N | — | NL input sheet + command pipeline | — |
| 61 | 043 | B | Opus | N | — | Block drag-and-drop reorder | — |
| 62 | 046-W | B | Opus | N | — | Vesper evening draft/approve loop + completion wiring | F1 C |
| 63 | 036-W | B | Opus | N | — | First-plan real synthesis trigger wiring | — |
| 64 | 049 | B | Sonnet | N | — | Fitness module (template wiring, scaling, detail UI, swap) | — |
| 65 | 050 | B | Opus | N | — | Nutrition module (recipe UI, hydration) | — |
| 66 | 051 | B | Opus | N | — | Meal planning + grocery list | — |
| 67 | 055 | B | Opus | N | — | Task placement algorithm (task → focus block) | — |
| 68 | 056 | B | Opus | N | — | Task reflow + over-commit prompt (voice-gated) | — |
| 69 | 058 | B | Sonnet | N | — | Weekly planning steps 4–5 + weekly grid | — |
| 70 | 059a | B | Opus | N | — | Sleep module logic (wind-down, bedtime, quiet hours) | — |
| 71 | 062 | B | Opus | N | — | Errands module | — |
| 72 | 065 | B | Opus | N | — | GCal push webhook channel + receiver | — |
| 73 | 066 | B | Opus | N | — | GCal channel renewal worker | — |
| 74 | 067 | B | Opus | N | — | GCal conflict resolution + regen prompt | — |
| 75 | 084 | B | Opus | N | — | Stripe webhook handler (worker) | — |
| 76 | 086 | B | Opus | N | — | Apple receipt verification API | — |
| 77 | 086a | B | Opus | N | — | Apple PKI monitor worker | — |
| 78 | 087 | B | Opus | N | — | Apple Server Notifications V2 — part 1 | — |
| 79 | 088 | B | Opus | N | — | Apple Server Notifications V2 — part 2 | — |
| 80 | 079 | B | Opus | N | — | expo-live-activities JS bridge | — |
| 81 | 075 | B | Opus | N | — | Bill reminder + APNs token cleanup workers | — |
| 82 | 080 | B | Opus | N | — | Live Activity pusher worker + immediate trigger | — |
| 83 | 071 | B | Opus | N | — | Cache pre-warm worker | — |
| 84 | 082 | B | Opus | N | — | Read-only mode enforcement (cross-cutting) | — |
| 85 | 089-W | B | Opus | N | — | Subscription lifecycle wiring (state machine + Stripe/Apple) | — |
| 86 | 090 | B | Opus | N | — | Cancellation flow + reason + deletion UI | — |
| 87 | 095-W | B | Opus | N | — | Referral attribution + cookie + mint-on-active + settings | F3 C |
| 88 | 092 | B | Opus | N | — | Resend templates 2 (waitlist, post-cancel, referral) | — |
| 89 | 094 | B | Opus | N | — | Landing SEO / OG + sitemap + icons | — |
| 90 | 072 | B | Opus | N | — | Trial reminder + dunning workers | — |
| 91 | 073 | B | Opus | N | — | Hard-delete worker + Stripe cleanup | — |
| 92 | 074 | B | Opus | N | — | Reconciliation worker | — |
| 93 | 105a | B | Opus | N | — | Demo account provisioning (2 reviewer accounts) | — |
| 94 | 096 | B | Opus | N | — | PostHog event taxonomy + 3 funnels | — |
| 95 | 097 | B | Opus | N | — | Sentry coverage + analytics + dashboard | — |
| 96 | 097a | B | Opus | N | — | Operational alerting + spend monitoring | — |
| 97 | 098 | B | Opus | N | — | Voice-gate end-to-end sweep | — |
| 98 | 099 | B | Opus | N | — | Error states 1 (AI fallback, offline, integration) | — |
| 99 | 100 | B | Opus | N | — | Error states 2 (regen, absence, 404, auth, mini-onboarding) | — |
| 100 | 101 | B | Opus | N | — | Accessibility + performance audit | — |
| 101 | 101a | B | Opus | N | — | Load test + chaos drill | — |
| 102 | 102 | B | Opus | N | — | App Store metadata + privacy manifest | — |
| 103 | 103 | B | Opus | N | — | App Store screenshots + marketing assets | — |
| 104 | 104 | B | Opus | N | — | Privacy policy + ToS live pages | — |
| 105 | 105 | B | Opus | N | — | TestFlight build + LA verification + submission | — |
| 106 | 105b | B | Opus | N | — | App Store review rejection response (conditional) | — |

---

## Fable-Window Note (EO, tracks, slack, pressure valve)
...
|---|---|---|
| **1** | Front matter: title, status/scope, sources, How-to-Use, Notation Legend (incl. Model field), per-chat template, master reordered-sequence table, Fable-window note + pressure valve, this file-split plan | this part — conventions source |
...
**Goal:** Build the design system as the first design-BUILD chat: a single source-of-truth token layer (color, type, motion, spacing, radii) expressed as a Tailwind theme for web and a React Native theme for mobile, plus the core primitive components every later screen composes from — cards, buttons, the block row, and the butler-line container. This chat also emits the native token values and a visual spec for the surfaces Opus owns in Swift, so visual consistency carries to native without spending Fable capacity on Swift.
...
- `DesignTokens.swift` token values plus a written visual spec for the iOS alarm screen (059b) and the three Live Activity widget variants (078), handed to Opus — Fable does not write the Swift itself
...
**Dependencies:** Built schema foundation (chats 004–006), the API foundation (chat 008), and the connection/user-filter pattern (chat 007). Runs on the build track at EO 12, ahead of the Fable card 108a (EO 24) so the card has a real API to call as soon as its wiring half is reached.
...
*Butler's Notebook · EO 24 · Model Fable · 🔵 🟢 · Skills: frontend-design · 🎩 · Window Y (Fwin F13) · CD-flags: —*
...
**Goal:** Build the notebook's visual card surface on the Fable design track: a single card in the espresso/cream/bronze system that states one inference in butler voice and offers a confirm and a correct affordance, with no list accumulation and no insights-dashboard chrome. The card is built static/mocked in the window, composing the 107/107a primitives; wiring it to the 108 API is deferred to the build track after Fable leaves.
...
*Block 0 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟡 · Skills: context-engineering-kit, caveman · — · Window N · CD-flags: —*
...
*Block 0 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟡 · Skills: superpowers · — · Window N · CD-flags: —*
...
*Block 0 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 1 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🗄️ 🟡 · Skills: drizzle-best-practices · ⚠️ · Window N · CD-flags: —*
...
*Block 1 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 1 · EO 4 · Model Opus · 🗄️ 🟡 · Skills: drizzle-best-practices · — · Window N · CD-flags: F1 R, F2 R, F3 R*
...
Consequently, for every carried-over chat in this part that falls in the 008–020 range, the meta line records **EO** as `— (built foundation; not in master sequence)` and **Model** as `Opus (build-track default per legend)`, exactly as the Part 2 note prescribes; no EO numbers are fabricated for them. Package colors, skill flags, risk flags, and all body content are carried over from the prior plan unchanged except for the reformat into the Part 1 per-chat template. The chats in this part that **do** carry their real master-table values are **021** (EO 6), **023** (EO 7), **022** (EO 27), and the new **112** (EO 5).
...
*Block 2 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 2 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🎩 🔵 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 2 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟢 · Skills: — · ⇄ 010 · Window N · CD-flags: —*
...
*Block 2 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🟢 · Skills: — · ⇄ 012 · Window N · CD-flags: —*
...
*Block 2 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🔵 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 3 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🤖 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 3 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🤖 🎩 🟡 · Skills: caveman, stop-slop · ⚠️ · Window N · CD-flags: —*
...
*Block 3 · EO — (built foundation; not in master sequence) · Model Opus (build-track default per legend) · 🤖 🎩 🟡 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 3 · EO 6 · Model Opus · 🤖 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 3 · EO 27 · Model Opus · 🤖 🟡 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 3 · EO 7 · Model Opus · 🤖 🎩 🟡 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 3 · EO 5 · Model Opus · 🤖 🎩 🟡 · Skills: caveman, stop-slop · — · Window N · CD-flags: F4 R*
...
**Block 5 split convention.** Block 5's chats 032–036 are each split into a Fable visual/static half (`NNN-V`, inside the Fable window) and an Opus wiring half (`NNN-W`, in normal build-track sequence). **Both halves are documented here, inside Block 5** — the split is a sequencing/ownership device, not a relocation, exactly as the Part 1 splits note prescribes [doc:PHASE_4_BUILD_PLAN_part1.md]. Each half takes its **own** EO, Model, and Window values from the master table: the `-V` halves are Fable, `Window Y`, and carry their own `Fwin F#` counter position (032-V = F5, 033-V = F6, 034-V = F7, 035-V = F8, 036-V = F9, per the Fable-window note); the `-W` halves are Opus on the build track, `Window N`, at their normal EO positions. The pairs are presented `-V` then `-W` so any reference to `NNN` resolves to the pair. A `-V` half is design-locked, wiring-deferred — not "done" until its `-W` half ships.
...
*Block 4 · EO 8 · Model Opus · 🔵 · Skills: — · — · Window N · CD-flags: —*
...
*Block 4 · EO 34 · Model Opus · 🔵 · Skills: — · ⚠️ · Window N · CD-flags: F1 C*
...
*Block 4 · EO 29 · Model Opus · 🔵 · Skills: — · ⇄ 025 · Window N · CD-flags: —*
...
*Block 4 · EO 35 · Model Opus · 🔵 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 4 · EO 30 · Model Opus · 🔵 · Skills: — · ⇄ 027 · Window N · CD-flags: —*
...
*Block 4 · EO 32 · Model Opus · 🔵 🤖 🎩 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 4 · EO 31 · Model Opus · 🔵 · Skills: — · — · Window N · CD-flags: —*
...
*Block 4 · EO 9 · Model Opus · 🔵 · Skills: — · ⇄ 030 · Window N · CD-flags: F3 C*
...
**Implementation notes:** This is a window chat (Fwin F5) and a Fable design-track session; it builds visual surfaces against the design system from 107/107a and mocks the reveal/skip state locally. **The 🎩 flag is propagated here, to the copy-authoring half:** the welcome lines and the honorific footnote are user-facing butler copy that must clear the caveman/stop-slop voice gate at authoring time, so the visual half owns the gate interaction; 032-W adds no new copy and shows `Skills: —`. The auth screen is a reposition of the already-built chats 010/011 surfaces, not new auth code; this half handles only its placement and styling within the onboarding flow. **Window vs CD-flag scoping:** the `Fwin F5` token is the fifth window-chat position and is unrelated to any `CD-flag F#`; this chat carries no CD-flag. Per the Fable-window note, every window chat depends only on already-built foundations plus the design system earlier in the same cluster, so this half is unblocked once 107a lands and runs start-to-finish inside the ≤11-day window while Opus builds the backend spine in parallel [doc:PHASE_4_BUILD_PLAN_part1.md].
...
*Block 5 · EO 46 · Model Opus · 🟡 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 5 · EO 17 · Model Fable · 🔵 🟢 · Skills: — · — · Window Y (Fwin F6) · CD-flags: —*
...
**Implementation notes:** Window chat (Fwin F6), Fable design-track. The tile UI is the durable visual surface; the branch logic and the PUT /profile write are deferred to 033-W. No user-facing freeform copy beyond the tile labels (which are fixed product nomenclature), so no 🎩 flag. **Window vs CD-flag scoping:** `Fwin F6` is the sixth window-chat position; this chat carries no CD-flag.
...
*Block 5 · EO 47 · Model Opus · 🟡 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 5 · EO 18 · Model Fable · 🔵 🟢 · Skills: — · — · Window Y (Fwin F7) · CD-flags: —*
...
*Block 5 · EO 52 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 5 · EO 19 · Model Fable · 🔵 🟢 🎩 · Skills: caveman, stop-slop · — · Window Y (Fwin F8) · CD-flags: —*
...
*Block 5 · EO 53 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 5 · EO 20 · Model Fable · 🔵 🟢 🎩 · Skills: caveman, stop-slop · — · Window Y (Fwin F9) · CD-flags: —*
...
*Block 5 · EO 63 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 6 · EO 45 · Model Opus · 🟡 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 6 · EO 48 · Model Opus · 🔵 · Skills: — · — · Window N · CD-flags: —*
...
*Block 6 · EO 49 · Model Opus · 🟢 · Skills: — · ⇄ 039 · Window N · CD-flags: —*
...
*Block 6 · EO 21 · Model Fable · 🔵 🟢 · Skills: — · — · Window Y (Fwin F10) · CD-flags: —*

**Load at session start:** PRD §4 and §6 (each module's block-detail content); the committed `BlockDetailsSchema` (the `block_details` JSONB Zod discriminated union from chat 006); the design system and component library from 107/107a (earlier in the same Fable window); `docs/DESIGN_STRATEGY.md` from chat 106.

**Goal:** Build the expanded block-detail layouts for each of the ten `block_type` values (work, fitness, nutrition, sleep, errands, medication, finance, focus, commute, custom) as static, design-locked surfaces composed from the 107a primitives and typed against the committed `BlockDetailsSchema`. This is the visual half: each layout renders against mocked or schema-shaped sample data, with no retrieval wiring — the binding to real retrieved block data is deferred to 041-W. As a Fable-window chat (Fwin F10) it depends only on already-built foundations plus the design system and runs while Opus builds the backend spine in parallel.
...
*Block 6 · EO 50 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 6 · EO 51 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 6 · EO 61 · Model Opus · 🔵 🟢 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 6 · EO 22 · Model Fable · 🔵 🟢 🟡 🎩 · Skills: caveman, stop-slop · — · Window Y (Fwin F11) · CD-flags: —*
...
**Implementation notes:** The library is authored as a TypeScript constant with each line tagged by context; the selection algorithm scores each candidate by context-match strength and recency, and the highest-scoring non-recent line wins. The no-repeat rule tracks the last shown line per context bucket and checks its timestamp against the current time minus four hours. **The 🎩 flag is propagated here, to the copy-authoring half:** the eighty butler lines are user-facing butler copy cleared through the caveman/stop-slop voice gate at authoring time, so the visual half owns the gate interaction; 044-W performs only the mechanical honorific substitution, adds no new copy, and shows `Skills: —`. **This chat is the pre-agreed pressure valve for the Fable window:** per the Part 1 Fable-Window Note, if the ≤11-day window slips, 044-V (Fwin F11) is the designated drop-to-14 because its visual plus local rotation is the lowest-leverage of the fifteen window chats and its function survives composition from 107a primitives during 044-W; dropping it loses no conversion, retention, onboarding, or design-foundation surface (108a Fwin F13 is the named alternate valve). The Fable-Window Note is canonical for that decision [doc:PHASE_4_BUILD_PLAN_part1.md]. **Window vs CD-flag scoping:** `Fwin F11` is the eleventh window-chat position and is unrelated to any `CD-flag F#`; this chat carries no CD-flag, read field-scoped per the Part 1 naming caveat.
...
*Block 6 · EO 59 · Model Opus · 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 6 · EO 60 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 6 · EO 23 · Model Fable · 🔵 🟢 🎩 · Skills: caveman, stop-slop · — · Window Y (Fwin F12) · CD-flags: —*
...
*Block 6 · EO 62 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: F1 C*
...
**Fold convention (052 and 054).** Block 7's chats 052 and 054 were originally marked for a `-V`/`-W` split, but per the Part 1 master table and Stage 2 §5 their Fable visual halves (**052-V** calendar chrome, **054-V** sortable task list) are **folded into their wiring halves** and are **not** in the Fable window — they are utilitarian, lowest-design-leverage surfaces trivially composable from the 107/107a primitives, and cutting them from the window is what brought the cluster from 17 candidates down to the 15 it holds [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md]. They are therefore rendered here as **single chats, 052-W and 054-W**, each taking its own master-table EO and Model (052-W EO 41, 054-W EO 43; both Opus) with the folded-in visual noted in prose — **not** as separate `-V`/`-W` pairs. The fold is the master table's and Stage 2's decision propagated here, not a new product decision.
...
**New chats 059a / 059b.** The original 059 work is rendered as two chats, each with its own master-table EO and Model: **059a** (sleep module logic) is EO 70, Model Opus on the build track; **059b** (iOS alarm screen) is EO 13, Model **Opus-native** — Opus owns the native SwiftUI alarm screen and Fable supplies only the visual spec via 107, per Stage 2 §1 [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md]. They are documented separately below at their respective positions.
...
**Two Model discrepancies (flagged, not reconciled).** The master table assigns **Sonnet** to chats **049** (EO 64) and **058** (EO 69) [doc:PHASE_4_BUILD_PLAN_part1.md], and both are ⚠️ in the prior plan and the Risk Map [doc:PHASE_4_BUILD_PLAN.md]. The Part 1 legend states ⚠️ chats are "held at Opus regardless of any lighter assignment," which would override Sonnet to Opus. Because every meta line must match the authoritative master table, the meta lines below read **Model Sonnet** for both, and the legend's ⚠️→Opus hold rule is recorded as a conflicting note inside each chat — the master-table-vs-legend gap is flagged, not patched.
...
Block 7 implements the seven modules — fitness, nutrition, sleep, medication, finance, errands, and tasks (the work module) — plus the cross-module Sunday weekly planning session. The template seed scripts and seed migrations land early so fitness and nutrition template selection works in the module chats that follow (047 sources templates at EO 10; 048 deploys the seed migrations at EO 28). The built-in calendar and tasks surfaces (052-W, 053, 054-W) absorb their folded-in Fable visuals and compose from the 107/107a design system. Block organization is preserved for readability; the actual run order is the EO column of the Part 1 master table, not the chat-number order in which the entries appear below — the Block 7 chats are interleaved across EO 10 through EO 71 on the build track. The one late-binding back-reference in this block (056 → 057) is governed by the Critical Path section, which is canonical for it; no other Block 7 entry depends on a later-EO chat.

### Chat 047 — Seed Sourcing Scripts

*Block 7 · EO 10 · Model Opus · 🟡 · Skills: — · — · Window N · CD-flags: —*
...
*Block 7 · EO 28 · Model Opus · 🗄️ 🟡 · Skills: drizzle-best-practices · — · Window N · CD-flags: —*
...
*Block 7 · EO 64 · Model Sonnet · 🔵 🟢 🤖 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
**Implementation notes:** The fitness template selection runs as part of the plan-synthesis prompt; the selected template UUID is referenced in `block.details`. The contextual adaptation is a separate Sonnet call invoked when the available time slot is shorter than the template's standard duration; it takes the template and the available time and returns a modified version (fewer sets, shorter rest intervals). The workout swap is invoked via the natural-language command pipeline from chat 045 rather than a distinct gesture; `SwapWorkoutSheet` is the surface the command applier opens when the parser returns a `swap_workout` command type. Swap and the NL command path are the same feature with different surfaces (input is NL, surface is the sheet). The sheet presents the next-best matching template based on the user's modified preference. The 🤖 flag marks this as an AI-layer chat where prompt versioning matters (the contextual-adaptation call), but the Part 1 legend names no skill to invoke for 🤖, so it is carried in the meta cluster as a flag and `Skills: —`, not expanded into an invented skill name [doc:PHASE_4_BUILD_PLAN_part1.md]. **Model discrepancy, flagged not reconciled:** the master table assigns Model **Sonnet** to this chat (EO 64), and this meta line matches it; the prior plan and Risk Map mark this chat ⚠️, and the Part 1 legend states ⚠️ chats are held at Opus regardless of any lighter assignment [doc:PHASE_4_BUILD_PLAN_part1.md][doc:PHASE_4_BUILD_PLAN.md]. The two Part 1 statements conflict for this chat; per the requirement that every meta-line Model match the master table, the meta line reads Sonnet, and the build session resolves the conflict at execution time (the Risk Map rationale — the contextual-adaptation prompt is one of the harder secondary AI prompts and must produce structurally-correct output while meaningfully modifying a template's parameters — argues for the ⚠️ Opus hold). The discrepancy is recorded here, not silently reconciled.
...
*Block 7 · EO 65 · Model Opus · 🔵 🟢 🤖 · Skills: — · ⇄ 049 · Window N · CD-flags: —*
...
*Block 7 · EO 66 · Model Opus · 🔵 🟢 🤖 🎩 · Skills: caveman, stop-slop · ⚠️ · Window N · CD-flags: —*
...
**Implementation notes:** The meal plan is triggered during the Sunday weekly planning session (chat 057) or on user demand. The grocery aggregator combines duplicate ingredients (two recipes calling for 2 cups of rice each become 4 cups total). The grocery list is a flat checklist; no store routing is implemented. **The 🎩 flag is the copy-authoring marker:** the grocery-list line is user-facing butler copy cleared through the caveman/stop-slop voice gate at authoring time, so those two skills are named in the Skills field; the 🤖 flag marks the meal-synthesis AI call where prompt versioning matters and is carried as a flag without an invented skill name [doc:PHASE_4_BUILD_PLAN_part1.md]. This chat is held at Opus and run in plan mode per its ⚠️ flag — the Risk Map rationale is that the weekly Sonnet meal synthesis must produce a coherent week's worth of meals, not seven independent days, and coherence is hard to achieve via prompting alone; the master-table Model (Opus) and the ⚠️→Opus hold rule **agree** here, so there is no Model discrepancy. The session model is Opus even though the meal-synthesis call it authors targets Sonnet.
...
*Block 7 · EO 41 · Model Opus · 🔵 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 7 · EO 42 · Model Opus · 🟢 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
**Implementation notes:** If react-native-calendars supports RRULE, use it directly. If not, the in-app expansion pattern is: the `calendar_events` table stores the RRULE string and the start/end dates of the series; on read, the app expands the RRULE to individual instances using the `rrule` library; the expanded instances are not stored (only the RRULE is). This pattern is standard for calendar applications and is well documented. Held at Opus and run in plan mode per the ⚠️ flag — the Risk Map rationale is that RRULE recurrence on react-native-calendars is the open question, and if the library does not support it natively the in-app expansion pattern adds complexity; master Model (Opus) and the ⚠️→Opus hold rule agree — no discrepancy. EO 42 places this immediately after 052-W (EO 41), matching the hard dependency [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].
...
*Block 7 · EO 43 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 7 · EO 67 · Model Opus · 🟡 🤖 · Skills: — · — · Window N · CD-flags: —*
...
*Block 7 · EO 68 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 7 · EO 56 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 7 · EO 69 · Model Sonnet · 🔵 🟢 🤖 🎩 · Skills: caveman, stop-slop · ⚠️ · Window N · CD-flags: —*
...
*Block 7 · EO 70 · Model Opus · 🎩 🟡 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 7 · EO 13 · Model Opus-native · 🟢 · Skills: — · ⚠️ · Window N · CD-flags: —*

**Load at session start:** PRD §3.2 (Morning alarm); LAYER_4_EXPERIENCE_IDENTITY.md (alarm design); Apple Human Interface Guidelines for notifications; the 107 visual spec for the alarm screen (Fable supplies the visual spec; Opus implements it in SwiftUI).
...
**Implementation notes:** This chat involves manual Xcode work because Expo does not have first-class support for Notification Service Extensions; the prebuild generates the iOS project, then the founder adds the target manually in Xcode. The extension's UI is implemented in SwiftUI. The two-button layout with edge-to-edge buttons is achieved via the notification's category configuration. Dismissal-without-unlock requires the notification's `interruption-level` to be `time-sensitive` or `critical`; time-sensitive is the appropriate choice for normal alarms (critical is reserved for emergency alerts). The runbook is essential because every `expo prebuild --clean` wipes the manually-added target; recovery requires the documented step list rather than improvisation. **Model and ownership note:** the master-table Model is **Opus-native** (EO 13) — Opus owns the native SwiftUI alarm screen, with Fable supplying only the visual spec via 107, per Stage 2 §1 [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md]; the meta line matches it. Held at Opus and run in plan mode per the ⚠️ flag — the Risk Map rationale is that native iOS work via a Notification Service Extension is unfamiliar territory for most Expo developers and requires Xcode debugging; master Model (Opus-native) and the ⚠️→Opus hold rule agree — no discrepancy. EO 13 is early because the chat depends only on the built mobile shell (chat 013).
...
*Block 7 · EO 54 · Model Opus · 🔵 🟢 · Skills: — · ⚠️ · Window N · CD-flags: F2 C*
...
*Block 7 · EO 55 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 7 · EO 71 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
**No splits, no folds, no window chats in this part.** Per the Part 1 file-split plan and master table, Block 8 (063–067), Block 9 (071–075), and Block 10 (076–080) contain **no `-V`/`-W` splits and no folded chats** [doc:PHASE_4_BUILD_PLAN_part1.md]. Per the master table and Stage 2 §5, **no Block 8/9/10 chat is a Fable-window chat** — every meta line below reads `Window N`, and none carries a `Fwin F#` counter [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md][doc:PHASE_4_BUILD_PLAN_part1.md]. The 15 entries are 063, 064, 065, 066, 067, 071, 072, 073, 074, 075, 076, 077, 078, 079, 080 — exactly one chat per ID, each at its own master-table EO and Model.
...
**Models in this part.** Block 8 chat **064** is **Haiku** per the master table (EO 44) — the event-classification work is the lightest AI-layer call in the GCal chain and is the one chat the plan assigns to Haiku here [doc:PHASE_4_BUILD_PLAN_part1.md]. Block 10 chats **077** (EO 14) and **078** (EO 33) are **Opus-native** per the master table and Stage 2 §1 — Opus owns the SwiftUI / WidgetKit / ActivityKit native targets (the widget-extension setup and the three Live Activity view variants plus `DesignTokens.swift`), with Fable supplying only the visual spec via 107; Fable does not own native Swift even at low dependency [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md][doc:PHASE_4_BUILD_PLAN_part1.md]. Every other Part 7 chat is **Opus** on the build track.

**Model-vs-legend: no discrepancy in this part.** Several Part 7 chats are ⚠️ in the prior plan and the Risk Map (063, 067, 071, 073, 074, 077, 078, 080) [doc:PHASE_4_BUILD_PLAN.md]. The Part 1 legend states ⚠️ chats are "held at Opus regardless of any lighter assignment" [doc:PHASE_4_BUILD_PLAN_part1.md]. For every ⚠️ chat in this part the master-table Model is already **Opus** (or **Opus-native**, which is Opus on Swift), so the master table and the ⚠️→Opus hold rule **agree** — there is no master-table-vs-legend Model conflict to flag here (unlike Part 6's Sonnet chats 049 and 058). Haiku chat 064 is **not** ⚠️ in the prior plan, so the lighter assignment stands with no hold-rule collision.
...
**Meta-line emoji ordering (per Part 2/4/5/6).** A 🎩-only build chat leads with the skill emoji then colors (067: `🎩 🔵`); when 🤖 is present, colors lead then 🤖 (064: `🔵 🤖`; 071: `🟣 🤖`); a colors-only chat shows just the package color(s). No Fable visual/static half appears in this part, so the "colors then 🎩" Fable ordering is not exercised here.
...
*Block 8 · EO 37 · Model Opus · 🔵 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 8 · EO 44 · Model Haiku · 🔵 🤖 · Skills: — · — · Window N · CD-flags: —*
...
*Block 8 · EO 72 · Model Opus · 🔵 · Skills: — · 🚧 · Window N · CD-flags: —*
...
*Block 8 · EO 73 · Model Opus · 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 8 · EO 74 · Model Opus · 🎩 🔵 · Skills: caveman, stop-slop · ⚠️ · Window N · CD-flags: —*
...
*Block 9 · EO 83 · Model Opus · 🟣 🤖 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 9 · EO 90 · Model Opus · 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 9 · EO 91 · Model Opus · 🟣 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 9 · EO 92 · Model Opus · 🟣 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 9 · EO 81 · Model Opus · 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 10 · EO 14 · Model Opus-native · 🟢 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*

**Load at session start:** TECHNICAL_SPEC.md §7 (SwiftUI Widget Extension); LAYER_4_EXPERIENCE_IDENTITY.md (Dynamic Island design specifications, design tokens); the 107 native visual spec (Fable supplies the visual spec; Opus implements it in SwiftUI); Apple's WidgetKit and ActivityKit documentation.
...
*Block 10 · EO 33 · Model Opus-native · 🟢 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*

**Load at session start:** Chat 077 (widget extension target exists); LAYER_4_EXPERIENCE_IDENTITY.md (full Dynamic Island design with all three states); the 107 native visual spec / DesignTokens values (Fable supplies the spec; Opus implements in SwiftUI); Apple's ActivityKit and Dynamic Island design guidelines.
...
*Block 10 · EO 80 · Model Opus · 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 10 · EO 82 · Model Opus · 🟣 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*
...
**Models in this part.** Every Part 8 chat is **Opus** on the build track except the one Fable visual half **089-V** (EO 25), which is **Fable** on the F track per the master table [doc:PHASE_4_BUILD_PLAN_part1.md]. There are no Sonnet, Haiku, or Opus-native chats in Block 11.

**Model-vs-legend: no discrepancy in this part.** Six Part 8 chats are ⚠️ in the prior plan and the Risk Map (081, 082, 084, 086, 087, 088) [doc:PHASE_4_BUILD_PLAN.md]. The Part 1 legend states ⚠️ chats are "held at Opus regardless of any lighter assignment" [doc:PHASE_4_BUILD_PLAN_part1.md]. For every ⚠️ chat in this part the master-table Model is already **Opus**, so the master table and the ⚠️→Opus hold rule **agree** — there is no master-table-vs-legend Model conflict to flag here (unlike Part 6's Sonnet chats 049 and 058, or the general caveat that where the two disagree the master table is authoritative and the conflict is recorded, not patched). No Block 11 chat carries a lighter assignment to collide with the hold rule.
...
**Meta-line emoji ordering (per Part 2/4/5/6/7).** A 🎩-only build chat leads with the skill emoji then colors (090: `🎩 🔵 🟢`); a Fable visual/static half puts colors then 🎩 (089-V: `🔵 🟢 🎩`); every other chat is colors-only in package order. No 🤖 chat appears here, so the "colors then 🤖 then 🎩" ordering is not exercised in this part.
...
*Block 11 · EO 84 · Model Opus · 🔵 🟢 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 11 · EO 58 · Model Opus · 🔵 · Skills: — · — · Window N · CD-flags: —*
...
*Block 11 · EO 75 · Model Opus · 🟣 · Skills: — · ⚠️ · Window N · CD-flags: —*
...
*Block 11 · EO 57 · Model Opus · 🟢 · Skills: — · 🚧 · Window N · CD-flags: —*
...
**Implementation notes:** StoreKit 2 is the modern Apple in-app purchase framework (versus the legacy StoreKit 1 with receipt validation against verifyReceipt). Purchases return a `signedTransaction.jws` string that contains the full transaction data signed by Apple; this is the payload sent to the apple-verify endpoint. The StoreKit Configuration File is a JSON file that lets Xcode simulate purchases against fake products during development, removing the need for a real App Store Connect product configuration during early development. Real product configuration happens at Cutover step C-11, which is why this chat is **🚧 (cannot run to completion until the Cutover Block completes)** for its real-product path; the StoreKit Configuration File path is exercisable beforehand. **Model note:** the master-table Model is **Opus** (EO 57); this chat is not ⚠️ in the prior plan, so the Opus assignment stands with no hold-rule interaction. Although the surface is native-adjacent (iOS in-app purchase), the prior plan does not mark it Opus-native — it is a TypeScript bridge wrapper in `@vesper/mobile`, not a SwiftUI/WidgetKit/ActivityKit target — so the master-table **Opus** (not Opus-native) assignment is correct and there is no Stage-2 §1 native-ownership conflict to flag [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md][doc:PHASE_4_BUILD_PLAN_part1.md].
...
*Block 11 · EO 76 · Model Opus · 🔵 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*
...
*Block 11 · EO 77 · Model Opus · 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 11 · EO 78 · Model Opus · 🟣 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*
...
*Block 11 · EO 79 · Model Opus · 🟣 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*
...
*Block 11 · EO 25 · Model Fable · 🔵 🟢 🎩 · Skills: caveman, stop-slop · — · Window Y (Fwin F14) · CD-flags: —*
...
*Block 11 · EO 85 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 11 · EO 86 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 11 · EO 40 · Model Opus · 🟢 · Skills: — · — · Window N · CD-flags: —*
...
**Models in this part.** Every Block 12 chat is **Opus** on the build track except the two Fable visual halves **093-V** (EO 15) and **095-V** (EO 26), which are **Fable** on the F track per the master table [doc:PHASE_4_BUILD_PLAN_part1.md]. There are no Sonnet, Haiku, or Opus-native chats in Block 12.

**FLAG — master-table Model vs the legend's ⚠️→Opus hold on 093-V.** The prior plan marks **093** as ⚠️ (Three.js mobile-performance risk on the brand-defining landing surface) and the Risk Map carries it [doc:PHASE_4_BUILD_PLAN.md]. The Part 1 legend states ⚠️ chats are "held at Opus regardless of any lighter assignment" [doc:PHASE_4_BUILD_PLAN_part1.md]. The performance-sensitive work is the Three.js hero, which lives in the **visual half 093-V** — and the master table assigns **093-V to Fable** as a window chat (EO 15) [doc:PHASE_4_BUILD_PLAN_part1.md]. The legend's ⚠️→Opus hold and the master-table Fable assignment therefore **disagree** on 093-V. Per the Part 1 rule that, where the master table and the legend disagree, **the master table is authoritative for the meta line and the conflict is recorded, not patched**, 093-V's meta line below carries **Model Fable** and the ⚠️ flag is retained in its risk field, with the conflict flagged in prose. No other Block 12 chat has a master-table-vs-legend Model conflict.
...
**Meta-line emoji ordering (per Part 2/4/5/8).** A 🎩-only build chat leads with the skill emoji then colors (091/092: `🎩 🟡`; 095-W: `🎩 🔵 🟢`); a Fable visual/static half puts colors then 🎩 (093-V: `🔵 🎩`; 095-V: `🔵 🎩`); colors-only chats are in package order (094: `🔵`; 096/097: `🔵 🟢 🟣`; 097a: `🟣`). No 🤖 chat appears in Block 12, so the "colors then 🤖 then 🎩" ordering is not exercised here.
...
*Block 12 · EO 88 · Model Opus · 🎩 🟡 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 12 · EO 15 · Model Fable · 🔵 🎩 · Skills: caveman, stop-slop · ⚠️ · Window Y (Fwin F4) · CD-flags: —*
...
*Block 12 · EO — (not in master table — see flag) · Model Opus (per Stage 2 §5; not a master-table row) · 🔵 · Skills: — · — · Window N · CD-flags: —*

**FLAG (entry-level):** This chat has **no row in the Part 1 master reordered-sequence table and no row in the Stage 2 §4 full reordered sequence** [doc:PHASE_4_BUILD_PLAN_part1.md][doc:PHASE_4_REPLAN_STAGE_2_REORDER.md]. Its existence and scope come only from Stage 2 §3, which splits 093 into a `-V` half and a `-W` half ("wire waitlist signup POST to 031"), and Stage 2 §5, which states that all `-W` wiring halves run post-window on the Opus build track. Because the master table is authoritative for the meta line but contains no 093-W row, its **EO is not determinable and is not invented** ("EO —"); its **Model Opus** and **Window N** are carried from Stage 2 §5's "all `-W` halves are post-window Opus" statement, with that weaker provenance flagged here. This is the same master-table-vs-Stage-2 discrepancy described in the Part 9a authoring note; it is surfaced, not reconciled.
...
**Implementation notes:** This wiring half adds no new user-facing copy — the landing copy and the confirmation/error strings came through the caveman/stop-slop voice gate in 093-V — so it carries no 🎩 and shows `Skills: —`, with the rationale "adds no new copy — the landing copy came through the gate in 093-V," matching the 089-W / 093-W-class wiring-half precedent [doc:PHASE_4_BUILD_PLAN_part8.md]. Because 093-V was built against the committed waitlist contract (the `{ email, platform }` shape POST /waitlist expects), the wiring is plumbing rather than redesign. **Model note:** Stage 2 §5 places all `-W` halves on the Opus build track, so this entry reads Model Opus; the **EO is left "—" because no master-table row sequences this chat** (see the entry-level flag). It composes 093-V and 031, both of which are sequenced (093-V at EO 15, 031 at EO 9), but its own position in the topological order is not recorded in the master table.
...
*Block 12 · EO 89 · Model Opus · 🔵 · Skills: — · — · Window N · CD-flags: —*
...
*Block 12 · EO 26 · Model Fable · 🔵 🎩 · Skills: caveman, stop-slop · — · Window Y (Fwin F15) · CD-flags: —*
...
*Block 12 · EO 87 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: F3 (C)*
...
*Block 12 · EO 94 · Model Opus · 🔵 🟢 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 12 · EO 95 · Model Opus · 🔵 🟢 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 12 · EO 96 · Model Opus · 🟣 · Skills: — · — · Window N · CD-flags: —*
...
**Implementation notes:** The alerting goal is "no surprise crises" rather than "monitor everything"; alerts that fire frequently get ignored. The thresholds here are starting points, tuned during the first production month based on actual signal-to-noise. The spend monitor is the highest-value alert because Anthropic spend is the largest non-fixed cost; a runaway prompt change or misbehaving worker could 10x daily spend overnight without any user-facing error. The 80% / 100% / 200% ladder allows graduated response. This chat authors no user-facing copy (operational alert config and a runbook), so it carries no 🎩 and shows `Skills: —`; it is worker/infra work, hence 🟣. **Model note:** the master-table Model is **Opus** (EO 96); the prior plan does not carry an ⚠️ symbol on 097a's meta line (the Risk Map discusses 097a's blast radius in prose, but the per-chat meta flag is the authoritative source for the meta line, and it is unflagged), so this entry carries no ⚠️ and the Opus assignment stands with no hold-rule interaction [doc:PHASE_4_BUILD_PLAN.md].
...
**No splits, no folds, no window chats in this part.** Per the Part 1 file-split plan and master table, Block 13 (098–105b) contains no `-V`/`-W` splits and no folded chats; no Block 13 chat is a Fable window chat, so every meta line reads `Window N` and carries no `Fwin F#` counter [doc:PHASE_4_BUILD_PLAN_part1.md]. The eleven rendered entries are 098, 099, 100, 101, 101a, 102, 103, 104, 105, 105a, 105b — each at its own master-table EO and Model. The Part 1 file-split plan's "21 entries" figure for Part 9 is the two-block total; the Block 13 contribution rendered here is **11 entries**, which the master table renders in full with no omission (the 093-W omission flagged in 9a is a Block 12 item).

**Models in this part.** Every Block 13 chat is **Opus** on the build track per the master table; there are no Fable, Sonnet, Haiku, or Opus-native chats in Block 13 [doc:PHASE_4_BUILD_PLAN_part1.md].

**Model-vs-legend: no conflict in this part.** Three Block 13 chats carry the ⚠️ symbol on their prior-plan meta lines — **098, 105a, 105** [doc:PHASE_4_BUILD_PLAN.md]. The Part 1 legend holds ⚠️ chats at Opus regardless of any lighter assignment; for all three the master-table Model is already **Opus**, so the master table and the ⚠️→Opus hold rule **agree** and there is no master-table-vs-legend conflict to flag here (unlike Block 12's 093-V) [doc:PHASE_4_BUILD_PLAN_part1.md]. Note that several chats discussed in the Risk Map prose (097a in Block 12; 101 here) do not carry an ⚠️ *symbol* on their meta line; the per-chat meta flag, not the Risk Map prose, is authoritative for the meta line, so 101 and 101a carry no ⚠️.
...
*Block 13 · EO 97 · Model Opus · 🎩 🔵 🟢 🟡 · Skills: caveman, stop-slop · ⚠️ · Window N · CD-flags: —*
...
*Block 13 · EO 98 · Model Opus · 🎩 🔵 🟢 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 13 · EO 99 · Model Opus · 🔵 🟢 🤖 🎩 · Skills: caveman, stop-slop · — · Window N · CD-flags: —*
...
*Block 13 · EO 100 · Model Opus · 🔵 🟢 · Skills: — · — · Window N · CD-flags: —*
...
*Block 13 · EO 101 · Model Opus · 🔵 🟣 · Skills: — · — · Window N · CD-flags: —*
...
*Block 13 · EO 102 · Model Opus · 🎩 🟢 · Skills: caveman, stop-slop · 🚧 · Window N · CD-flags: —*
...
*Block 13 · EO 103 · Model Opus · 🎩 🟢 · Skills: caveman, stop-slop · 🚧 · Window N · CD-flags: —*
...
*Block 13 · EO 104 · Model Opus · 🎩 🔵 · Skills: caveman, stop-slop · 🚧 · Window N · CD-flags: —*
...
*Block 13 · EO 105 · Model Opus · 🟢 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*
...
*Block 13 · EO 93 · Model Opus · 🟢 · Skills: — · ⚠️ 🚧 · Window N · CD-flags: —*
...
**Implementation notes:** The demo accounts are critical because Apple reviewers cannot complete real OAuth flows; without credentialed accounts they have no way past the sign-in screen, and the rejection reason will be "unable to evaluate." The seed walkthrough script is idempotent so it can be re-run before every submission to reset both accounts to a known good state; it must be tested before C-21 so the seed data is known-good for the App Store Review pass. The subscriptions are sandbox (Apple) and test-mode (Stripe); both are appropriate for App Review and do not consume real funds. This chat authors no new user-facing butler copy (provisioning and seeding), so it shows `Skills: —` and carries no 🎩. **Model and risk note:** the master-table Model is **Opus** (EO 93); this chat is ⚠️ in the prior plan and the Risk Map (without working demo credentials the rejection reason is "unable to evaluate" and the resubmission cycle costs 1-3 days), and the ⚠️→Opus hold rule agrees with the master-table Opus assignment, so there is no discrepancy [doc:PHASE_4_BUILD_PLAN_part1.md][doc:PHASE_4_BUILD_PLAN.md]. Held at Opus per ⚠️. Carries 🚧. **Sequencing note:** its EO (93) sits earlier in the order than chat 105 (EO 105) despite the 105a number, because the seeded reviewer accounts must exist before submission; the master table is canonical for sequencing and the chat-number order is for readability only [doc:PHASE_4_BUILD_PLAN_part1.md].
...
*Block 13 · EO 106 · Model Opus · 🟢 · Skills: — · 🚧 · Window N · CD-flags: —*
...
**Implementation notes:** Common rejection categories and their responses: guideline 4.8 sign-in compliance (handled by chats 010, 011 enabling Apple Sign In; should not recur but if it does, the fix is verifying the Apple Sign In button is functional and prominent); guideline 5.1.1 privacy (handled by chats 102, 041 PostHog masking; if it recurs, the response audits the App Privacy questionnaire against the actual data flows); guideline 2.1 information needed (handled by chat 105a demo account; if it recurs, the response provides additional walkthrough material). The typical resolution cycle is 1-3 days per round; budget two rounds before approval. This chat may not be needed if the initial submission is accepted directly. It authors no new user-facing butler copy (rejection diagnosis and resubmission), so it shows `Skills: —` and carries no 🎩. **Model note:** the master-table Model is **Opus** (EO 106, the last position in the order); the prior-plan meta line carries no ⚠️ symbol, so this entry carries no ⚠️ and the Opus assignment stands. Carries 🚧 (it acts on a Cutover-gated submission). It is conditional — it runs only on rejection.
...
The chats below carry elevated risk and require plan mode, multiple iterations if needed, and no shortcuts. Each is flagged with ⚠️ in its rendered meta line in parts 2–9; this map provides the prose explanation of why each chat is risky. The membership below is regenerated from the rendered chat headers across parts 2–9 as the source of truth [doc:PHASE_4_BUILD_PLAN_part1.md], rather than from memory.

Per the legend, ⚠️-flagged chats are held at the Opus model regardless of any lighter Sonnet/Haiku assignment [doc:PHASE_4_BUILD_PLAN_part1.md]. Two chats expose a conflict between that rule and the master table, recorded and flagged below rather than patched.
...
**Chat 049 — Fitness module: selection, adaptation, UI.** The Sonnet contextual adaptation prompt is one of the harder secondary AI prompts; it must produce structurally-correct output while modifying a template's parameters meaningfully. **Flagged conflict:** the master table assigns this chat the Sonnet model (EO 64), while the legend says a ⚠️ chat is held at Opus regardless of a lighter assignment [doc:PHASE_4_BUILD_PLAN_part1.md]. Per the rule that the master table is authoritative for the meta line, the rendered header in part 6 keeps Sonnet [doc:PHASE_4_BUILD_PLAN_part6.md]; the conflict is recorded and flagged here, not patched. The remaining elevated-risk handling (plan mode, extra iterations) still applies.
...
**Chat 052-W — Built-in calendar library evaluation and web implementation (052-V folded in).** Library choice is hard to reverse. FullCalendar versus react-big-calendar is a real decision that affects every calendar surface forward. The master table folds 052-V into 052-W as a single chat (EO 41); the calendar chrome was deferred from the Fable window per Stage 2 §5 and rebuilt here composing the 107/107a primitives [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].
...
**Chat 058 — Weekly planning steps 4 and 5.** The Sonnet weekly template generation is the most complex AI call in the application; it produces seven days of plans simultaneously with priority threading. **Flagged conflict:** as with chat 049, the master table assigns Sonnet (EO 69) while the legend would hold a ⚠️ chat at Opus [doc:PHASE_4_BUILD_PLAN_part1.md]. The rendered header in part 6 keeps Sonnet per the master-table-authoritative rule [doc:PHASE_4_BUILD_PLAN_part6.md]; the conflict is recorded and flagged, not patched.

**Chat 059b — iOS alarm screen.** Native iOS work via Notification Service Extension is unfamiliar territory for most Expo developers and requires Xcode debugging. Opus owns this native Swift surface (built from the 107 visual spec, not by Fable) per Stage 2 §1; its model is Opus-native, with which the ⚠️ hold-at-Opus rule does not conflict [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].
...
**Chat 077 — SwiftUI Live Activity widget extension setup.** Manual Xcode work outside Expo's automation. Configuration errors result in build failures or runtime failures that are hard to diagnose. Opus owns this native surface per Stage 2 §1; it carries 🚧 in its rendered meta line and cannot ship until Cutover provisions the App Group, provisioning profile, and APNs key [doc:PHASE_4_BUILD_PLAN_part7.md].

**Chat 078 — SwiftUI Live Activity widget UI.** SwiftUI itself is a learning curve for developers used to React Native. The widget's three view variants must each render correctly and respect the design tokens emitted as `DesignTokens.swift` from chat 107. Native Opus ownership; carries 🚧.
...
**Chat 093-V — Waitlist landing page with Three.js cinematic (visual / static).** Performance-sensitive brand-defining surface. Slow rendering on mobile devices destroys conversion. The risk lives on the Fable visual/static half (EO 15, Window Y, Fwin F4), which builds the five-section page and the Three.js cinematic hero on canned templates with no account; the wiring half (093-W) only attaches the signup POST to chat 031 and carries no ⚠️ [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].
...
Above the block structure sits the re-plan's two-track axis: the Fable (F) design track and the Opus/Sonnet (B) build track run in parallel across the same calendar [doc:PHASE_4_BUILD_PLAN_part1.md]. The fifteen window chats form a self-contained F-track chain (106 → 107 → 107a, then the `-V` halves and 108a/089-V/095-V) that runs start-to-finish while the build track works the backend spine; no window chat depends on any unbuilt build-track chat, so the two tracks never block each other inside the window [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].

**Design Cluster:** 106 → 107 → 107a, sequential on the F track (each depends on the prior). They precede every screen chat and every `-V` half by construction.
...
**Block 5:** Each screen chat is split into a Fable visual/static `-V` half (in the window) and an Opus wiring `-W` half (on the build track). The `-V` halves 032-V → 033-V → 034-V → 035-V → 036-V run in Fwin order F5–F9 inside the window, composing the 107/107a primitives; the `-W` halves slot into build-track EO order (032-W EO 46, 033-W EO 47, 034-W EO 52, 035-W EO 53, 036-W EO 63) and are sequenced by their wiring dependencies rather than by screen order [doc:PHASE_4_BUILD_PLAN_part4.md]. In particular 034-W follows 063 and 064 (Critical Path 063 → 064 → 034) and 036-W follows the real-synthesis trigger (025).
...
**Block 7:** 047 first. 048 follows 047. 049 and 050 parallel after 048 (⇄). 051 follows 050. 052-W (with 052-V folded in) is the calendar entry; 053 follows 052-W. 054-W (with 054-V folded in) follows 028. 055 follows 054-W. 056 follows 055 (and hard-depends on 057; Critical Path 056 → 057). 057 first for weekly planning; 058 follows 057. 059a and 059b parallel after 035-W (059b is native Opus, EO 13, pulled early). 060, 061, 062 parallel after 028.
...
Every other chat in Phase 4 parallelizes around this spine. With three to four terminals running concurrently, total Phase 4 calendar time is approximately the spine length divided by daily session throughput, with the Fable window cluster running fully in parallel on the F track. The additional chats (086a, 090b, 097a, 101a, 105b) add roughly five non-critical-path sessions; 090a is deferred to V1.5 and is not part of Phase 4; 105b runs only if Apple rejects the initial submission.
...
The re-plan sequences 109 carried-over chats (non-deferred) plus 13 split halves plus 7 new chats into 106 execution-ordered positions; the deferred V1.5 export chat 090a is excluded from the order [doc:PHASE_4_BUILD_PLAN_part1.md]. The build runs on two parallel tracks — the Fable design track and the Opus/Sonnet build track — with fifteen chats held inside the ≤11-day Fable window (Fwin F1–F15) and 044-V designated as the pre-agreed pressure valve, 108a as the alternate [doc:PHASE_4_BUILD_PLAN_part1.md]. Native Swift surfaces (059b, 077, 078) are built by Opus from a Fable-authored visual spec rather than by Fable [doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].
...
- **093-W has no master-table row.** Its EO is left "—" and its model follows Stage 2 §5 (Opus); it is not a master-table-sequenced entry [doc:PHASE_4_BUILD_PLAN_part9a.md][doc:PHASE_4_REPLAN_STAGE_2_REORDER.md].
...
- **⚠️-but-Sonnet conflict.** Chats 049 and 058 carry ⚠️ in their rendered headers while the master table assigns them Sonnet; the legend's hold-at-Opus rule and the master table disagree. The master table is authoritative for the meta line, so the headers keep Sonnet; the conflict is recorded and flagged in the Risk Map.
...
*Block 11 · EO — (deferred to V1.5; not in master sequence) · Model Opus (build-track default per legend) · 🔵 🟣 · Skills: caveman, stop-slop · 🎩 · Window N · CD-flags: —*
...
**Known carried discrepancy from part 9 — do NOT "fix" it during assembly.** Chat 093-W has no master-table row; its EO is left "—" and its model follows Stage 2 §5 (not a master-table row) [doc:PHASE_4_BUILD_PLAN_part9a.md]. The File-Split Plan's "21 entries" figure for part 9 is a pre-fold/pre-split count that disagrees with the master-table-rendered count; this is flagged, not reconciled [doc:PHASE_4_BUILD_PLAN_part1.md]. The assembler must preserve both as-is. The remaining carried discrepancies are catalogued in the Closing Notes above and are likewise to be preserved, not patched.
