// Pinned Apple Root CA accessor for StoreKit 2 / ASSN V2 JWS verification (§8).
//
...
//   * NO JWKS fetch. The apple-verify / apple-assn flow is x5c-certificate-chain
//     verification, not a key-set lookup. We never hit appleid.apple.com/auth/keys
//     (that endpoint is for Sign In with Apple identity tokens — a different flow).
//   * NO per-transaction network call to Apple. The JWS x5c header carries the whole
//     leaf→intermediate→root chain, so verification is fully self-contained; the only
//     trust anchor we need is the pinned root, which is a build-time constant.
