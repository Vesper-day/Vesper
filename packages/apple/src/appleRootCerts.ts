// Pinned Apple Root CA certificates for StoreKit 2 / App Store Server Notifications
// V2 JWS verification (§8).
//
// These are the trust anchors the apple-verify route AND the apple-assn worker
// validate the JWS x5c chain against. This is CERTIFICATE PINNING, not a
// JWKS/network lookup: the StoreKit transaction / ASSN V2 notification JWS carries
// its full leaf→intermediate→root chain in the x5c header, and we refuse to trust
// that traveling root — we require the chain to terminate in one of the roots pinned
// HERE (§8: "pinned in code").
//
...
//
// SINGLE SOURCE OF TRUTH (Chat 087): this constant was extracted verbatim from the
// former apps/web/lib/apple/appleRootCerts.ts into @vesper/apple so BOTH apps/web
// (via a thin re-export shim) and the apple-assn worker import the same pinned root.
// The apps/web file is now `export * from '@vesper/apple'`; the 086a PKI monitor
// parity test (workers/apple-pki-monitor/index.test.ts) still reads APPLE_ROOT_CA_PINS
// through that shim, so the worker's re-pinned copy stays byte-locked to this source.
