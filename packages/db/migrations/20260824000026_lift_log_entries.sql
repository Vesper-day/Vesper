...
-- derivation the hydration counter uses, migration 14 / supabase mirror 000019); there
-- is no stored daily total. DEEP fitness columns (bronze->platinum strength-rank;
-- world-standard percentile mapping) are DEFERRED (PRD §6.2) and intentionally NOT added
-- here — the module page reserves a named layout slot for them and ships none.

BEGIN;

-- lift_log_entries -----------------------------------------------------------
--   exercise_name        free-text or template-sourced exercise name.
--   workout_template_id  set when logging against a scheduled/selected workout;
--                        NULL for ad-hoc. ON DELETE SET NULL so removing a corpus
--                        workout does not delete the user's log row.
--   set_number           1-based set index (CHECK > 0).
--   reps                 optional; CHECK >= 0 when present.
--   weight               optional; numeric(7,2); CHECK >= 0 when present.
--   weight_unit          'kg' or 'lb'; NULL for bodyweight sets.
